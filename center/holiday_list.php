<?
	...
	if ($data_execute != ''){
		...
		if ($data_execute == 'save'){
			...
			for($i=0; $i<$count; $i++){
				$pay_yn	= $_POST['pay_yn_'.$i];
				if ($del_yn[$i] != 'Y'){
					$sql = "replace into m06holiday (m06_ccode, m06_date, m06_name, m06_pay_yn) values ('".$code."', '".str_replace('-', '', $date[$i])."', '".$name[$i]."', '".$pay_yn."')";
				}else{
...
<!--
my_column = new Array(new Array('no', 'no'), new Array('date[]', 'date', 'focus'), new Array('name[]', '', 'focus'), new Array('pay_yn', 'radio', 'add', new Array(new Array('Y','유급', ''), new Array('N', '무급', 'checked'))), new Array('delete','button'));
	...
	<?
		...
		for($i=0; $i<$row_count; $i++){
			...
			<tr id="row_<?=$i+1;?>">
				...
				<td class="center">
					<input name="pay_yn_<?=$i;?>" type="radio" class="radio" value="Y" <? if($pay_yn == 'Y'){?>checked<?} ?>>유급
					<input name="pay_yn_<?=$i;?>" type="radio" class="radio" value="N" <? if($pay_yn != 'Y'){?>checked<?} ?>>무급
				</td>
				<td class="left last">

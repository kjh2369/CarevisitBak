		...
		<tr>
			...
			<?
				...
				for($i=0; $i<$rowCount; $i++){
					...
					echo "<option value='".$row[0].'_'.$row[2]."' ".($row[0] == $person[0] ? "selected" : "")." >".$row[1]."</option>";
				}
...
<table class="my_table my_border" style="margin-top:-1px;">
	...
	<?
		...
		if ($_SESSION["userLevel"] == "A"){
			...
			if ($find_person != '')  $wsl .= " and b02_person = '".$find_person."'";																//담당자명만 검색
		...
		if ($row_count > 0){
			for($i=0; $i<$row_count; $i++){
				...
				<tr>
					...
					<td class="left"><a href="#" onclick="_reg_center('<?=$row['code'];?>');"><?=$row['nm'];?></a></td>
					...
					<?
						if ($_SESSION['userLevel'] == 'A'){?>
							...
							<td class="left last"><a href="#" onclick="showCenterScreen('<?=$gDomainID;?>','http://<?=$url;?>/main/login_ok.php','<?=$ed->en($row['id']);?>','<?=$ed->en($row['pw']);?>');">바로가기</a></td><?
						}else{?>
							<td class="other">&nbsp;</td><?
						}
					?>

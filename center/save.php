<?
	...
	for($i=0; $i<=sizeOf($code_2); $i++){
		...
		if ($code_value != ''){
			...
			if ($conn->get_data($sql) == 0){
				$sql = "insert into m00center (m00_mcode, m00_mkind, m00_code1, m00_cname, m00_jdate, m00_domain) values ('$code', '$i', '$code_value', '$name_value', '$j_date', '$gDomain')";
			...
			$sql = "update m00center
					...
					,      m00_day_work_hour	  = '".$_POST['day_work_hour'.$i]."'
					,      m00_day_hourly         = '".str_replace(',', '', $_POST['day_hourly'.$i])."'
	...
	$sql = "select count(*)
			 ...
			 where m97_user = '$code'";
	if ($conn->get_data($sql) == 0){
		$sql = "insert into m97user (m97_user,m97_id,m97_pass) values ('$code','$code','1111')";
		$conn->execute($sql);
	}
	...
	if ($conn->get_data($sql) == 0){
		...
		$sql = 'insert into m01suga (m01_mcode, m01_mcode2, m01_scode, m01_suga_cont, m01_suga_value, m01_suga_value15, m01_suga_value75, m01_suga_cvalue1, m01_suga_cvalue2, m01_suga_cvalue3, m01_calc_time, m01_sdate, m01_edate, m01_rate) '
			 ...
			 . 'select \''.$code.'\', m01_mcode2, m01_scode, m01_suga_cont, m01_suga_value, m01_suga_value15, m01_suga_value75, m01_suga_cvalue1, m01_suga_cvalue2, m01_suga_cvalue3, m01_calc_time, m01_sdate, m01_edate, m01_rate'
			 . '  from m01suga'

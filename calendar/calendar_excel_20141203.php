<?
	...
	for($i=0; $i<$rowCnt; $i++){
		...
		$data[$row['day']][] = Array(
			'time'=>($row['fulltime_yn'] == 'Y' ? '종일' : $row['from_time'])
		,	'text'=>$row['subject']
		);
	}
	...
	<body>
		<table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse; table-layout:fixed;">
			...
			<tr style="height:3px;">
				<td style="width:43px; border:none;">&nbsp;</td>
			...
			for($i=1; $i<=$lastday; $i++){
				...
				//ROWS
				if ($startWeekday == 0 || $startWeekday == $startWeekly){
					$rows = 1;
				...
				<tr><?
					for($ii=0; $ii<7; $ii++){
						if (is_array($data[$tmpI][$j])){?>
							...
							<td colspan="2" style="border:0.5pt solid BLACK; border-top:none; text-align:center; vertical-align:middle;"></td><?
						}
						$tmpI ++;
					}?>
				</tr><?

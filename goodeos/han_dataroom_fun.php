	...
	if ($type == 'DATAROOM'){
		...
		if ($mode == 'ATTACH_DELETE'){
			...
			*/
			...
			header("Content-Length: ".(string)(filesize('../_stats/community/attach_file/'.$row['file_path'])));
			header("Cache-Control: cache, must-revalidate");
			header("Pragma: no-cache");
			header("Expires: 0");

			$fp = fopen('../_stats/community/attach_file/'.$row['file_path'], 'rb');
			while(!feof($fp)){

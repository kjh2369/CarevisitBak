<?
	...
	*/
	...
	$sql = "select m00_day_work_hour, m00_day_hourly
			  from m00center
	...
	$day_work_hour = $center['m00_day_work_hour'];
	$day_hourly = $center['m00_day_hourly'];
	...
	for($i=0; $i<=$row_count; $i++){
		...
		if($jumin != ''){
			$mobile = preg_replace ("/[ #\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $row[$i][2]); //핸드폰
			$addr1 = $row[$i][3]; //주소
			$addr2 = $row[$i][4]; //주소
			$form	= preg_replace ("/[ #\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $row[$i][5]); //고용형태
			$inDate =  preg_replace ("/[ #\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $row[$i][6]); //입사일
			$yFamCareUmu = (strstr($row[$i][7],'유') != '' ? 'Y' : 'N'); // 동거케어유무
			$bohumYN = (strstr($row[$i][8],'유') != '' ? 'Y' : 'N'); //4대보험가입여부

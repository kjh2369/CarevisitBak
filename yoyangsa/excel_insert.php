	...
	*/
	...
	for($i=3; $i<=$excel->sheets[0]['numRows']; $i++){
		...
		if($jumin != ''){
			...
			if($excel->sheets[0]['cells'][$i][18] == '한국은행'){$bankName = '001';}
			else if($excel->sheets[0]['cells'][$i][18] == '산업은행'){$bankName = '002';}
			else if($excel->sheets[0]['cells'][$i][18] == '기업은행'){$bankName = '003';}
			else if($excel->sheets[0]['cells'][$i][18] == '국민은행'){$bankName = '004';}
			else if($excel->sheets[0]['cells'][$i][18] == '외환은행'){$bankName = '005';}
			else if($excel->sheets[0]['cells'][$i][18] == '수협중앙회'){$bankName = '007';}
			else if($excel->sheets[0]['cells'][$i][18] == '수출입은행'){$bankName = '008';}
			else if($excel->sheets[0]['cells'][$i][18] == '농협중앙회'){$bankName = '011';}
			else if($excel->sheets[0]['cells'][$i][18] == '농협회원조합'){$bankName = '012';}
			else if($excel->sheets[0]['cells'][$i][18] == '우리은행'){$bankName = '020';}
			else if($excel->sheets[0]['cells'][$i][18] == 'SC제일은행'){$bankName = '023';}
			else if($excel->sheets[0]['cells'][$i][18] == '한국씨티은행'){$bankName = '027';}
			else if($excel->sheets[0]['cells'][$i][18] == '대구은행'){$bankName = '031';}
			else if($excel->sheets[0]['cells'][$i][18] == '부산은행'){$bankName = '032';}
			else if($excel->sheets[0]['cells'][$i][18] == '광주은행'){$bankName = '034';}
			else if($excel->sheets[0]['cells'][$i][18] == '제주은행'){$bankName = '035';}
			else if($excel->sheets[0]['cells'][$i][18] == '전북은행'){$bankName = '037';}
			else if($excel->sheets[0]['cells'][$i][18] == '경남은행'){$bankName = '039';}
			else if($excel->sheets[0]['cells'][$i][18] == '새마을금고연합회'){$bankName = '045';}
			else if($excel->sheets[0]['cells'][$i][18] == '신협중앙회'){$bankName = '048';}
			else if($excel->sheets[0]['cells'][$i][18] == '상호저축은행'){$bankName = '050';}
			else if($excel->sheets[0]['cells'][$i][18] == '우체국'){$bankName = '071';}
			else if($excel->sheets[0]['cells'][$i][18] == '하나은행'){$bankName = '081';}
			else if($excel->sheets[0]['cells'][$i][18] == '신한은행'){$bankName = '088';}
			...
			$insToDate = preg_replace ("/[ #\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $excel->sheets[0]['cells'][$i][24]) != '' ? preg_replace ("/[ #\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $excel->sheets[0]['cells'][$i][24]) : '99999999'; //보험만료일

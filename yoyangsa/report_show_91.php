<?
	...
	$pdf->SetXY(14, 40);
	//$pdf->SetFillColor('255');
	$pdf->Rect(14, 40, 182, 135);
	$pdf->SetLineWidth(0.2);
	$pdf->Line(13.3,39.3,196.7,39.3);
	$pdf->Line(13.3,175.7,196.7,175.7);
	$pdf->Line(13.3,39.3,13.3,175.7);
	$pdf->Line(196.7,39.3,196.7,175.7);
	...
	$pdf->text(30,70,'문란케 하거나 재산상의 손실을 주었을 때.');
	...
	$pdf->SetFont('바탕','B',9);
	$pdf->text(25,110,'"을"은 "갑" 회사의 사규를 열람하고, 본 계약서 및 취업규칙에 없는 사항에 대해서는 노동관계법령 및 제규정과');
	$pdf->text(25,115,'"갑"의 해석에 따르기로 한다.');

	$pdf-> text(93,145,substr($row["m02_yipsail"],0,4).'년  '.substr($row["m02_yipsail"],4,2).'월  '.substr($row["m02_yipsail"],6,2).'일');

	$pdf->SetFont('굴림','B',10);
	$pdf->text(25,160,'사용자(갑)');
	$pdf->text(112,160,'근로자(을)');
	$pdf->SetXY(44,153.8);
	...
	$pdf->text(79,160,'(서명 또는 인)');
	$pdf->text(165,160,'(서명 또는 인)');

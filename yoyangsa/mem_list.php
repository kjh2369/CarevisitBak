...
<table class="my_table my_border">
	...
	<tbody>
		...
		<tr>
			...
			<td class="left other" style="padding-left:20px;">
				<span class="btn_pack m icon"><span class="refresh"></span><button type="button" onclick="_listMember('1');">조회</button></span>
				<span class="btn_pack m icon"><span class="reg"></span><button type="button" onclick="_mem_reg('<?=$find_center_code;?>','','');">등록</button></span>
				<span class="btn_pack m icon"><span class="excel"></span><button name="btnExcel" type="button" onFocus="this.blur();" onClick="_yoy_chk_popup('<?=$find_center_name?>','<?=$find_yoy_name?>','<?=$find_yoy_phone?>','<?=$find_yoy_stat?>','<?=$find_dept?>','<?=$find_ssn?>');">엑셀</button></span>
			</td>
		</tr>
	</tbody>
</table>
						...
						<?
							...
							$sql = "select m02_ccode
									...
									,	   case m02_ygoyong_kind when '1' then '정규직'
																 when '2' then '계약직' else ' ' end as m02_ygoyong_kind

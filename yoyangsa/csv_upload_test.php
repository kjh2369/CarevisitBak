<?
	...
	$mFile = date('Ymd', mkTime()).'_'.date('His', mkTime()).'.csv';
	...
	if ($f['tmp_name'] != ''){
		if (move_uploaded_file($f['tmp_name'], $mFile)){
			// 업로드 성공
			$upload = true;
		}else{
			// 업로드 실패
	...
	echo '
		<script language="javascript">
			location.replace("csv_insert_test.php?p_code='.$mCode.'&p_kind='.$mKind.'&p_file='.$mFile.'&duplicateYN='.$mDuplicate.'");
		</script>
		 ';
?>
<?
	...
	$sql = 'SELECT	mst.addr
			...
			WHERE seq = (select max(seq) from tablet_request as tmp where tmp.org_no = tb.org_no)
			  AND del_flag = \'N\'';
...
?>
	<tr style="background-color:#<?=$bgcolor;?>;">
		...
		<td class="center" ><?=number_format($tot_rank);?> 대</td>
		<td class="right ">
		...
		<td colspan="3" class="right last" style="font-size:9pt; font-weight:bold;">입금대수: <?=number_format($DIPO_CNT);?> 대&nbsp; 입금액 : <?=number_format($DIPO_TOT);?> 원</td>
	</tr>
<?

...
function _store_codepage() {
	...
	function caption($p_size, $p_color = 'black', $p_hAlign = 'center', $p_vAlign = 'vcenter'){
		$format =& $this->addformat(array(
										bold    => 1,
										color   => $p_color,
										size    => $p_size,
										merge   => 1,
										align	=> $p_vAlign));
		$format->set_align($p_hAlign);
		return $format;
	}

	function cell_format($p_textColor = 'black',
						 $p_textBold = false,
						 $p_textSize = 10,
						 $p_backColor = 'white',
						 $p_borderColor = 'black',
						 ...
						 $p_numFormat = '',
						 $p_pattern = 0x1){
		if ($p_textColor == '') $p_textColor = 'black';
		if ($p_backColor == '') $p_backColor = 'white';
		if ($p_borderColor == '') $p_borderColor = 'black';
		if ($p_hAlign == '') $p_hAlign = 'left';
		if ($p_vAlign == '') $p_vAlign = 'vcenter';

		$cell =& $this->addformat();
		$cell->set_color($p_textColor);
		$cell->set_size($p_textSize);
		$cell->set_pattern($p_pattern);
		$cell->set_fg_color($p_backColor);
		$cell->set_border_color($p_borderColor);
		$cell->set_align($p_hAlign);
		$cell->set_align($p_vAlign);

		if ($p_borderTop > 0) $cell->set_top($p_borderTop);
		if ($p_borderBottom > 0) $cell->set_bottom($p_borderBottom);
		if ($p_borderLeft > 0) $cell->set_left($p_borderLeft);
		if ($p_borderRight > 0) $cell->set_right($p_borderRight);

		if ($p_textBold == true) $cell->set_bold();
		if ($p_marge == true) $cell->set_merge();
		if ($p_textWrap == true) $cell->set_text_wrap();

		if ($p_numFormat != '') $cell->set_num_format($p_numFormat);

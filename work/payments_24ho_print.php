<?
	...
	$sql .=	  "   and t13_type     = '2'"
			. " group by m03_name, m03_injung_no, m03_jumin, t13_pay_date, t13_bill_no";
	...
	for($j=0; $j<$row_count; $j++){
		...
		*/
		...
		//echo $mCode.' / '.$mKind.' / '.$sugupjaJumin.' / '.$mDate.' / '.$mBoninYul.' / '.$billNo.' / '.date('Ymd',mkTime()).'<br>';
		...
		$pdf->SetFont('굴림','b',11);
		$pdf->Cell(46,	10,	$misy_yn != 'Y' ? number_format($boninPay) : '',	1,	2,	'R');
		$pdf->SetFont('굴림','',11);
		...
		# 푸터
		$pdf->SetFont('굴림','b',10);
		$pdf->Text(14, 280, '본인부담금 납입계좌: '.$bankName.' '.$bankNo.' 예금주 : '.$bankDepos);
		$pdf->Text(14, 284, '전화 : '.$centerTel);

<?
	...
	<div class="title_border"><?
		...
		<div class="title" style="float:left; width:auto;">일 실적 등록(수급자)</div>
	</div>
	...
	<table class="my_table" style="width:100%;">
		...
		<tbody>
			...
			if ($order == '1'){
				...
				$sql .=' order by conf_nm1, conf_from, conf_to, plan_from, plan_to, kind';
			}else if ($order == '5'){
			...
			for($i=0; $i<$rowCount; $i++){
				...
				if($order == 1){
					//수급자별 백그라운드 색 변경
					if($tmp[$i-1]['jumin'] != $tmp[$i]['jumin']){
						$row_back_cnt ++;
					}
				}else if($order == 4){
					//담당자별 백그라운드 색 변경
					if($tmp[$i-1]['conf_cd1'] != $tmp[$i]['conf_cd1']){
						$row_back_cnt ++;
					}
				}
				...
				$lbEdit = true;?>
					<tr id="trVal_<?=$i?>" style="background-color:<?=$row_color;?>"
						jumin		="<?=$ed->en($jumin);?>"
						svcCd		="<?=$row['kind'];?>"
						svcKind		="<?=$row['kind_cd'];?>"
						date		="<?=$row['date'];?>"
							   ...
							   $tmp[$i]['conf_from'] != '' ){
								if($order == 1){
									if($tmp[$i-1]['jumin'] == $tmp[$i]['jumin'] ){
										...
										echo '<span style="color:'.$color.';">'.$diff_mins.'</span>';
									}
									
								}else if($order == 4){
									if($tmp[$i-1]['conf_cd1'] == $tmp[$i]['conf_cd1'] || 
									   $tmp[$i-1]['conf_cd1'] == $tmp[$i]['conf_cd2'] ||
									   $tmp[$i-1]['conf_cd2'] == $tmp[$i]['conf_cd1'] ){
										$confTo = $tmp[$i-1]['conf_to'];
										$confFrom = $tmp[$i]['conf_from'];
										$diff_mins=((strtotime($confFrom)-strtotime($confTo))/60).'분'; 
										//$rest_hours = $total_secs % 86400; 
			...
			if ($('#txtConfFrom_'+lsIdx).val() && $('#txtConfTo_'+lsIdx).val()){
				...
				,	success: function(result){
						...
						$(obj).attr('sugaCd',val['code'])
							  ...
							  .attr('sugaVal',liVal);
						$('td', $(obj)).eq(5).css('color','#0000ff').text(val['name']);
						...
						$('td', $(obj)).eq(6).css('color','#0000ff').text(__num2str(orgAmt));
	...
	function lfSetTot(){
		...
		$('tr[id^="trVal_"][svcCd="0"][bipayYn!="Y"][svcKind!="200"]').each(function(){
			...
			liPlanAmt += __str2num($('td', $(this)).eq(7).text());
			liConfAmt += __str2num($('td', $(this)).eq(6).text());
	...
	function IfExcel(){
		...
		f.action = './result_detail_excel.php?mode=DAY_EXCEL&order=<?=$order;?>';
		f.submit();
	}

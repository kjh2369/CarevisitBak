<?
	...
	*********************************************************/
	...
	$weekly_pay		= str_replace(',', '', $_POST['weekly_pay']);	//주휴수당
	$paid_pay		= str_replace(',', '', $_POST['paid_pay']);		//유급수당
	...
	*/
	...
	if ($pay_list_my[$jumin]['ins_yn'] == 'Y'){
		if ($m_age >= 60){
			/*********************************************************
				만60세 이상은 입력한 국민연금 금액을 입력한다.
			*********************************************************/
			$worker_annuity	= str_replace(',', '', $_POST['pension_amt']); //국민연금 본인부담
			$center_annuity	= 0; //국민연금 기관부담
		}else{
			/*********************************************************
				만60세미만은 국민연금 금액을 계산한다.
			*********************************************************/
	...
	/*********************************************************
		...
		갑근세와 주민세는 입력한 값으로 사용한다.
	*********************************************************/
		$gapgeunse = str_replace(',', '', $_POST['tax_amt_1']);
		$juminse   = str_replace(',', '', $_POST['tax_amt_2']);
	/********************************************************/

<?
	...
	for($i=0; $i<$row_count; $i++){
		...
		$salary_addon[$row['salary_type']][$index[intval($row['salary_type'])]]['pay']		= $row['salary_index'] > 0 ? $row['salary_pay'] : 0;

		if ($is_preview == true){
			//$addon_pay[$row['salary_type']] += $row['salary_pay'];
			$addon_pay[$row['salary_type']] += ($row['salary_index'] > 0 ? $row['salary_pay'] : 0);
		}
	}

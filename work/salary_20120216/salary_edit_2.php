<?
	...
	$page		= $_REQUEST['page'];
	$showGbn	= $_REQUEST['showGbn'];
	$is_table	= $_REQUEST['is_table'];
	...
	//if (!$salary['rank_pay'] && $edit_mode) $salary['rank_pay'] = $rank_pay;
	...
	for($i=0; $i<$row_count; $i++){
		...
		for($j=0; $j<$temp_addon_count[$row['salary_type']]; $j++){
			if ($salary_addon[$row['salary_type']][$j]['index'] == $row['salary_index']){
				$salary_addon[$row['salary_type']][$j]['subject'] = $row['salary_subject'];
...
<?
	if ($edit_mode == 1){?>
		...
		function sum_bojeon(){
			...
			var base_max        = __str2num(f.base_max.value);
			...
			if (bojeon_max + base_max < meal_pay + car_keep_pay){
				alert('식대보조비와 차량유지비의 합이 보전수당인 '+__commaSet(base_max + bojeon_max)+'을 넘을 수 없습니다. 확인하여 주십시오.');
				meal_pay     = __str2num(f.meal_pay.tag);
				car_keep_pay = __str2num(f.car_keep_pay.tag);
				result = false;
			}

			var tmp_bojeon_pay = bojeon_max - (meal_pay + car_keep_pay);

			if (tmp_bojeon_pay < 0){
				f.base_pay.value   = __num2str(base_max + tmp_bojeon_pay);
				f.bojeon_pay.value = 0;
			}else{
				f.base_pay.value   = __num2str(base_max);
		...
		*********************************************************/
		function set_ins_amt(){
			var tot_ins_pay = __str2num(document.getElementById('pension_amt').value)
							+ __str2num(document.getElementById('health_amt').value)
							+ __str2num(document.getElementById('care_amt').value)
							+ __str2num(document.getElementById('employ_amt').value);

			document.getElementById('tot_ins_pay').value = __num2str(tot_ins_pay);
			sum_diff();
		}
		...
		*********************************************************/
		function set_tax_amt(){
			var tax_amt_1   = __str2num(document.getElementById('tax_amt_1').value);
			var tax_amt_2   = cut(tax_amt_1 * 0.1, 10);
			var tot_tax_pay = tax_amt_1 + tax_amt_2;

			document.getElementById('tax_amt_2').value   = __num2str(tax_amt_2);
			document.getElementById('tot_tax_pay').value = __num2str(tot_tax_pay);
			sum_diff();
		}
	...
	<script language='javascript'>
		...
		function list(){
			...
			if ('<?=$is_table;?>' == '1'){
				f.action = 'salary_table.php?year=<?=$year;?>&month=<?=$month;?>&page=<?=$page;?>';
			}else{
...
<div style="width:auto; font-weight:bold; margin-top:9px; text-align:right;"><?=$edit_mode == 2 ? '※ 급여가 마감되었습니다.' : '';?></div>
<?
if ($edit_mode != 3){?>
				...
				<tr>
					...
					<?
						for($i=1; $i<=12; $i++){
							...
							echo '<div class=\''.$class.'\' style=\'float:left; margin-right:'.$margin_right.';\'>'.$link.'</div>';
						}
					?>
					</td>
					<td class="last"></td>
	...
	<tbody>
		...
		<tr>
			...
			<td class="center border_b">
				<input name="base_pay" type="text" value="<?=number_format($salary['base_pay']);?>" class="number readonly" alt="not" readonly>
				<input name="base_max" type="hidden" value="<?=number_format($salary['base_max']);?>">
			</td>
			...
			<td class="center border_b"><input name="meal_pay" type="text" value="<?=number_format($salary['meal_pay']);?>" tag="<?=$salary['meal_pay'];?>" class="number readonly" style="background-color:#f6f4d3;" onchange="return limit_no(this, <?=__MAX_MEAL_AMT__;?>, '식대보조비 최대금액은 <?=__MAX_MEAL_AMT__;?>입니다.','sum_bojeon()');" <? if($edit_mode != 1){?>readonly<?} ?>></td>
			<td class="center border_b"><input name="car_keep_pay" type="text" value="<?=number_format($salary['car_keep_pay']);?>" tag="<?=$salary['car_keep_pay'];?>" class="number readonly" style="background-color:#f6f4d3;" onchange="return limit_no(this, <?=__MAX_CARKEEP_AMT__;?>, '차량유지비 최대금액은 <?=__MAX_CARKEEP_AMT__;?>입니다.','sum_bojeon()');" <? if($edit_mode != 1){?>readonly<?} ?>></td>
			<td class="center border_b">
				<input name="bojeon_pay" type="text"   value="<?=number_format($salary['bojeon_pay']);?>" class="number readonly" alt="not" readonly>
				<input name="bojeon_max" type="hidden" value="<?=$salary['bojeon_max'];/*$salary['bojeon_pay']+$salary['meal_pay']+$salary['car_keep_pay'];*/?>">
			</td>
	...
	<tbody>
		...
		<tr>
			...
			<?
				if ($m_age >= 60){
					/*********************************************************
						 만 60세가 넘으면 국민연금 금액을 수정할 수 있도록한다.
					*********************************************************/?>
					<input name="pension_amt" type="text" value="<?=number_format($salary['pension_amt']);?>" class="number readonly" style="width:98%; background-color:#f6f4d3;" onchange="set_ins_amt();" <? if($edit_mode != 1){?>readonly<?} ?>><?
				}else{
					/*********************************************************
						만 60세미만은 계산된 국민연금금액을 사용한다.
					*********************************************************/?>
					<input name="pension_amt" type="text" value="<?=number_format($salary['pension_amt']);?>" class="number readonly" style="width:98%;" alt="not" readonly><?
				}
			?>
			</td>
...
<div style="width:auto; text-align:right; float:right;">
	<table class="my_table" style="border-bottom:none;">
		...
		<tbody>
			<tr>
				<th class="head" rowspan="2" style="border-left:1px solid #a6c0f3;"><?=$salary['paye_yn'] == '1' ? '원천징수' : '소득세';?></th>
				<th class="head">갑근세</th>
				<th class="head">주민세</th>
				<th class="head last border_left border_right">합계(D)</th>
			</tr>
			<tr>
				<td class="center border_b"><input name="tax_amt_1" type="text" value="<?=number_format($salary['tax_amt_1']);?>" class="number readonly" style="width:98%; background-color:#f6f4d3;" onchange="set_tax_amt();" <? if($edit_mode != 1){?>readonly<?} ?>></td>
				...
				<table class="my_table my_border" style="width:100%; border-top:none;">
					...
					<tbody>
						<tr>
							...
							<td class="center last"><input name="rank_pay" type="text" value="<?=number_format($salary['rank_pay']);?>" tag="<?=$salary['rank_pay'];?>" class="number readonly" style="background-color:#f6f4d3;" onchange="sum_give();" <? if($edit_mode != 1){?>readonly<?} ?>></td>
						</tr><?
						...
						for($i=0; $i<$count; $i++){?>
							<tr>
								...
								<td class="center last"><input name="1_pay[]" type="text" value="<?=number_format($salary_addon[1][$i]['pay']);?>" tag="<?=$salary_addon[1][$i]['pay'];?>" class="number readonly" style="background-color:#f6f4d3;" onchange="sum_give();" <? if($edit_mode != 1){?>readonly<?} ?>></td>
					...
					<?
						...
						for($i=0; $i<$count; $i++){?>
							<tr>
								...
								<td class="center last"><input name="2_pay[]" type="text" value="<?=number_format($salary_addon[2][$i]['pay']);?>" tag="<?=$salary_addon[2][$i]['pay'];?>" class="number readonly" style="background-color:#f6f4d3;" onchange="sum_deduct();" <? if($edit_mode != 1){?>readonly<?} ?>></td>
...
<input type="hidden" name="showGbn"  value="<?=$showGbn;?>">

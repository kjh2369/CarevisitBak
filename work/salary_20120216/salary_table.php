<?
	...
	$printDay = $printDay != '' ? $printDay : date('Ymd', mktime());
...
function excel(){
	...
	location.replace('salary_table_excel.php?code='+code+'&kind='+kind+'&year='+year+'&month='+month+'&ex_gbn=1');
}
...
function excel2(){
	...
	location.replace('salary_table_excel.php?code='+code+'&kind='+kind+'&year='+year+'&month='+month+'&ex_gbn=2');
}
...
function excel3(){
	...
	location.replace('salary_table_excel.php?code='+code+'&kind='+kind+'&year='+year+'&month='+month+'&ex_gbn=3');
}

function pdf(){
	showMyReport('17', document.getElementById('code').value, document.getElementById('kind').value, document.getElementById('year').value, document.getElementById('month').value, '', 'month', document.getElementById('printDay').value);
}
...
<table class="my_table my_border">
	...
	<tbody>
		<tr>
			...
			<td class="right last">
				<span> 출력일:
					<input name="printDay" type="text" style="width:80px;" maxlength="11" class="date" onKeyDown="__onlyNumber(this)" onFocus="__replace(this, '-', '');" onBlur="__getDate(this);" onClick="_carlendar(this);" value="<?=$myF->dateStyle($printDay)?>">
				</span>
		...
		<tr>
			<td class="right last" colspan="6"><?
				if($debug){ ?>
					<span class="btn_pack m icon"><span class="excel"></span><button type="button" onclick="excel3();">Excel1</button></span><?
				}else { ?>
					<span class="btn_pack m icon"><span class="excel"></span><button type="button" onclick="" disabled>Excel1</button></span><?
				} ?>
				<span class="btn_pack m icon"><span class="excel"></span><button type="button" onclick="excel2();">Excel2</button></span>
				<span class="btn_pack m icon"><span class="excel"></span><button type="button" onclick="excel();">Excel3</button></span>
			</td>
		</tr>
	</tbody>
</table>

<?
	...
	for($m_index=0; $m_index<$member_count; $m_index++){
		...
		$pdf->Cell($pdf->w * 0.16, $pdf->text_height, number_format($salary['work_time'], 1), 1, 0, 'R');
		...
		$pdf->set_default_xy(0, 5);
		$pdf->set_caption_font('B');
		$pdf->Cell($pdf->w * 0.18, $pdf->row_height + $pdf->text_height, $salary['paye_yn'] == '1' ? '원천징수' : '소득세', 1, 0, 'C', true);
		$pdf->set_caption_font('');

<?
	...
	<table class="my_table" style="width:100%;">
		...
		<tbody>
			<tr>
				...
				<td class="right last">
					<? if($debug){ ?><span class="btn_pack m icon"><span class="delete"></span><button type="button" onclick="lfSWDel();">복지사삭제</button></span><? } ?>
			...
			<tr>
				...
				<td class="last" colspan="4">
					<input id='order1' name='order' type='radio' value='1' class='radio' onclick='lfSearch();' <? if($order == '1'){ echo 'checked'; } ?> ><label for='order1'>일자순별</label>
					...
					<input id='order3' name='order' type='radio' value='3' class='radio' onclick='lfSearch();' <? if($order == '3'){ echo 'checked'; } ?> ><label for='order3'>담당자순별</label>
				</td>
			</tr>
		</tbody>
	</table>
	...
	<table class="my_table" style="width:100%;">
		...
		<tbody>
			...
			$sql = 'select m01_mcode2 as suga
					...
					,      service_cost as cost
					 ...
					 where org_no = \'goodeos\'
					   and left(replace(service_from_dt,\'-\',\'\'),'.strlen($year.$month).') <= \''.$year.$month.'\'
					   and left(replace(service_to_dt,\'-\',\'\'),  '.strlen($year.$month).') >= \''.$year.$month.'\'';
			...
			if($order == '1'){
				...
				$sql .=' order by plan_time desc, plan_to desc, kind';
			}else if($order == '3'){
				$sql .=' order by conf_nm1, kind';
			}
			//echo nl2br($sql); 
			$conn->query($sql);
			$conn->fetch();
			...
			for($i=0; $i<$rowCount; $i++){
				...
				$orgAmt  = $liConfAmt;
				$rate    = FloatVal($laRate[IntVal($row['date'])]);
				...
				$lbEdit = true;?>
					<tr id="trVal_<?=$i?>"
						...
						svcLvl		="<?=$row['kind'] == '0' ? $careLevel : $laVoucher[$row['kind']]['lvl'];?>"
						sugaCd		="<?=$lsSugaCd;?>"
						sugaNm		="<?=$lsSugaNm;?>"
						sugaVal		="<?=$liConfAmt;?>"
						saveVal		="0"
					...
					<td class="left" style="color:#000000;"><?=$lsSugaNm.($row['bipay_yn'] == 'Y' ? '(<span style=\'color:#ff0000;\'>비</span>)' : '');?></td><?
						...
						}else if ($row['kind'] == '4' && $row['kind_cd'] == '200'){?>
							<td class="right" style="color:#000000; background-color:#efefef; cursor:pointer;" onclick="lfReSetAmt(this,<?=$i;?>);"><?=Number_Format($liConfPay);?></td><?
						}else{?>
					...
					<td class="left"><?
						if ($lbEdit){?>
							...
							if (($row['kind'] == '0' && $row['kind_cd'] == '500') ||
								($row['kind'] == '0' && $row['kind_cd'] == '200' && $row['dmta_yn'] == 'Y') ||
	...
	$('input[name^="txtConf_"]').unbind('change').change(function(){
		...
		if ($(obj).attr('svcCd') == 'A' || $(obj).attr('svcCd') == 'B' || $(obj).attr('svcCd') == 'C'){
			...
			if ($(obj).attr('svcCd') == '0'){
				...
				if ($(obj).attr('svcKind') == '200' && $('#year').val()+$('#month').val() >= '201703'){
					if ($(obj).attr('svcLvl') >= '3'){
						if (liProc >= 270 && liProc <= 540){
						}else if (liProc > 209){
							//liTo = liFrom + 209;
							//$('#txtConfTo_'+lsIdx).val(__min2time(liTo, 'HHMM'));
						}
					}
				}
			...
			$('#lblConfTime_'+lsIdx).css('color','#0000ff').text(__num2str(liProc));
			...
			if ($('#txtConfFrom_'+lsIdx).val() && $('#txtConfTo_'+lsIdx).val()){
				...
				,	success: function(result){
						...
						if ($(obj).attr('svcKind') != '210' && ($(obj).attr('holidayYn') == 'Y' || $(obj).attr('weekly') == '1')){
							if ($(obj).attr('extraCharge') == 'Y'){
						...
						//$('td', $(obj)).eq(4).css('color','#0000ff').text(__num2str(Math.round((liVal - (liVal * (__str2num($(obj).attr('cutGbn')) * 0.01))) / 10) * 10));
						$('td', $(obj)).eq(4).css('color','#0000ff').text(__num2str(orgAmt));
	...
	function lfSave(){
		...
		$('[id^="trVal_"][addRow="Y"]').each(function(){
			...
			para = para
				 ...
				 + '&time='+$('#lblConfTime_'+lsIdx).text().split(',').join('')
	...
	function lfSWDel(){
		...
		if (!confirm('복지사 전체삭제를 하시겠습니까?')) return;
	...
	function lfReSetAmt(obj, i){
		var obj = $(obj).parent();

		if ($(obj).attr('extraCharge') == 'Y'){
			$(obj).attr('extraCharge','N');
		}else{
			$(obj).attr('extraCharge','Y');
		}
		$('#txtConfFrom_'+i).change();
	}
	...
	function lfConfCutSet(gbn,i){
		...
		$('td', $(obj)).eq(4).css('color','red').text(__num2str(orgAmt));
		$('#divTemp').html('');
		lfSetCutLabel();
	}
	...
	function lfSetCutLabel(){
		$('tr[id^="trVal_"]').each(function(){
			...
			if ($(this).attr('cutGbn') == '0'){
				...
				$('#lblCutGbn_'+i)
					.css('left',($('td',this).eq(4).offset().left+2)+'px')
					.css('top',($('td',this).eq(4).offset().top-7)+'px')
	...
	function IfExcel(){
		...
		f.action = './result_detail_excel.php?mode=excel';
		f.submit();
	}

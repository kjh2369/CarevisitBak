<?
	...
	if ($row_count > 0){
		...
		if($mode == 'excel'){ 
			//엑셀출력일 경우
			echo '<div class=\'title\'>바우처 및 기타유료 확정내역</div>';
			echo '<table class="my_table" border="1">';
			$css = 'head';
		}else {
			echo '<div class=\'title title_border\'>바우처 및 기타유료 확정내역</div>';
		...
		echo '<th class=\''.$css.'\'><u title=\'총본인부담금 = 본인부담금 + 초과금액 + 비급여\'>총본인부담금</u></th>';
		echo '</tr>';
		echo '</thead>';

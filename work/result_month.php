<?
	...
	*********************************************************/
	...
	$sql = 'SELECT jumin
			,      seq
			,      amt
			,      DATE_FORMAT(from_dt,\'%Y%m\') AS f_yymm
			,      DATE_FORMAT(to_dt,\'%Y%m\') AS t_yymm
	...
	$sql = 'select svc.jumin
			...
			,      date_format(lvl.to_dt,\'%Y%m\') as t_yymm
			  ...
			  left join client_his_lvl as lvl
				on lvl.org_no           = svc.org_no
			   and lvl.jumin            = svc.jumin
		...
		foreach($laClientLvl as $jumin => $row){
			foreach($row as $mon => $lvl){
				...
				/*
					...
					if ($yymm >= $laLimit[$jumin]['from'] && $yymm <= $laLimit[$jumin]['to']){
						if (!Empty($laLimit[$jumin]['amt'])){
							$laLimitPay[$jumin][$mon] = $laLimit[$jumin]['amt'];
						}
					}
				 */
				if (is_array($tmpLimitPay[$lvl])){
					foreach($tmpLimitPay[$lvl] as $idx => $tmp){
						if ($yymm >= $tmp['from'] && $yymm <= $tmp['to']){
							$laLimitPay[$jumin][$mon] = $tmp['pay'];
							break;
						}
					}
				}
...
function lfWorkOpen(code, kind, year, month, jumin){
	if ($('#otpPopY').attr('checked')){
		lfWorkPop(code, kind, year, month, jumin);
	}else{
		lfWorkList(code, kind, year, month, jumin);
	}

	return false;
}

function lfWorkList(code, kind, year, month, jumin){
	var f = document.f;
...
function lfWorkPop(code, kind, year, month, jumin){
    ...
    var url    = './result_detail_pop.php';
	var win    = window.open('', 'WORKLIST', option);
		win.opener = self;
		win.focus();
    ...
    form.setAttribute('target', 'WORKLIST');
...
<table class="my_table" style="width:100%;">
	...
	<tbody>
		<tr>
			...
			<td class="right last">
				<input id="otpPopY" name="otp" type="radio" value="" class="radio"><label for="otpPopY">새창</label>
				<input id="otpPopN" name="otp" type="radio" value="" class="radio" checked><label for="otpPopN">현재창</label>
			</td>
		</tr>
	</tbody>
</table>
...
<table class="my_table" style="width:100%;">
	...
	<?
		if ($mode == 1 || $mode == 2){
			...
			*********************************************************/
			...
			//서비스검색
			if($find_kind != 'all') $wsl2 .= " and t01_mkind = '".$find_kind."'";
					  ...
					  from (
						   ...
						   ,      sum(conf_var01) as conf_var01, sum(conf_var02) as conf_var02, sum(conf_var03) as conf_var03, sum(conf_var04) as conf_var04, sum(conf_var05) as conf_var05, sum(conf_var06) as conf_var06, sum(conf_var07) as conf_var07, sum(conf_var08) as conf_var08, sum(conf_var09) as conf_var09, sum(conf_var10) as conf_var10, sum(conf_var11) as conf_var11, sum(conf_var12) as conf_var12
							 from (
								  ...
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '01' and t01_del_yn = 'N' then 1 else 0 end) as m01
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '02' and t01_del_yn = 'N' then 1 else 0 end) as m02
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '03' and t01_del_yn = 'N' then 1 else 0 end) as m03
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '04' and t01_del_yn = 'N' then 1 else 0 end) as m04
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '05' and t01_del_yn = 'N' then 1 else 0 end) as m05
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '06' and t01_del_yn = 'N' then 1 else 0 end) as m06
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '07' and t01_del_yn = 'N' then 1 else 0 end) as m07
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '08' and t01_del_yn = 'N' then 1 else 0 end) as m08
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '09' and t01_del_yn = 'N' then 1 else 0 end) as m09
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '10' and t01_del_yn = 'N' then 1 else 0 end) as m10
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '11' and t01_del_yn = 'N' then 1 else 0 end) as m11
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '12' and t01_del_yn = 'N' then 1 else 0 end) as m12

								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '01' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat01
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '02' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat02
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '03' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat03
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '04' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat04
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '05' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat05
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '06' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat06
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '07' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat07
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '08' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat08
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '09' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat09
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '10' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat10
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '11' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat11
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '12' and t01_status_gbn = '1' and t01_del_yn = 'N' then 1 else 0 end) as stat12
								  ...
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '01' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var01
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '02' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var02
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '03' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var03
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '04' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var04
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '05' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var05
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '06' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var06
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '07' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var07
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '08' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var08
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '09' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var09
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '10' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var10
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '11' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var11
								  ,      sum(case when SUBSTRING(t01_sugup_date,5,2) = '12' and t01_status_gbn = '1' and t01_del_yn = 'N' and t01_bipay_umu != 'Y' then t01_conf_suga_value else 0 end) as conf_var12
									...
									for($j=1; $j<=12; $j++){
										...
										if (substr($laClt[$mst[$i]['jumin']]['i_date'],0,6) <= $find_year.$mon &&
											...
											if ($mst[$i]['m'.$mon] > 0){
												...
												$text = '<a href="#" onclick="return lfWorkOpen(\''.$mst[$i]['code'].'\',\''.$mst[$i]['kind'].'\',\''.$find_year.'\',\''.$mon.'\',\''.$ed->en($mst[$i]['jumin']).'\');" style=\''.$style.'\'>'.$j.'월</a>';
											}else{
												$class .= 'my_month_2 ';
												$text   = '<span style="color:#7c7c7c; cursor:default;">'.$j.'월</span>';
											}
										}else{
											$text = '<span>&nbsp;</span>';
										}?>
									...
									for($j=1; $j<=12; $j++){
										...
										if ($mst[$i]['i_date'] <= $find_year.$mon && $mst[$i]['o_date'] >= $find_year.$mon){
											if ($mst[$i]['m'.$mon] > 0){
												...
												$text = '<a href="#" onclick="lfWorkList(\''.$mst[$i]['code'].'\',\''.$mst[$i]['kind'].'\',\''.$find_year.'\',\''.$mon.'\',\''.$ed->en($mst[$i]['jumin']).'\');" style=\''.$style.'\'>'.$j.'월</a>';
											}else{
												$class .= 'my_month_2 ';
	...
	<tbody>
		<tr>
			<td class="bottom last" colspan="5">&nbsp;</div>
			</td>
		</tr>
	</tbody>
</table>

...
<div class="title">월 실적 등록(<?=$title;?>)</div>
...
<table class="my_table my_border" style="margin-top:-1px; border-bottom:none;">
	...
	<?
		if ($mode == 1 || $mode == 2){
			$wsl = " and m03_mkind = (select min(chd.m00_mkind) from m00center as chd where chd.m00_mcode = mst.m03_ccode)
					 and '$find_year' between left(m03_gaeyak_fm, 4) and left(case when length(m03_gaeyak_to) = 8 then m03_gaeyak_to else '99999999' end, 4)";
		...
		if ($mode == 1 || $mode == 2){
			$sql = "select m03_ccode as code
					...
					,      sum(case date_format(t01_sugup_date, '%m') when '01' then 1 else 0 end) as m01
					...
					,     (select count(*) from t13sugupja where t13_ccode = t01_ccode and t13_mkind = t01_mkind and t13_jumin = t01_jumin and t13_pay_date = '".$find_year."12' and t13_type = '2') as conf12
					 ...
					 inner join t01iljung
					  ...
					  left join closing_progress
						on org_no       = t01_ccode
					   and closing_yymm = left(t01_sugup_date, 6)
					 where m03_ccode = '$code'
					   and m03_del_yn = 'N' $wsl
					 group by m03_ccode, m03_mkind, m03_jumin, m03_name
					 order by m03_name
			...
			$sql = "select m02_ccode as code
					...
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."01') as conf01
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."02') as conf02
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."03') as conf03
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."04') as conf04
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."05') as conf05
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."06') as conf06
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."07') as conf07
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."08') as conf08
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."09') as conf09
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."10') as conf10
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."11') as conf11
					,     (select count(*) from salary_basic where org_no = t01_ccode and salary_jumin = t01_yoyangsa_id1 and salary_yymm = '".$find_year."12') as conf12
					  from m02yoyangsa as mst
					 inner join t01iljung
						on t01_ccode         = m02_ccode
					   and t01_mkind         = m02_mkind
					 ...
					 where m02_ccode = '$code'
					   and m02_del_yn = 'N' $wsl
					 group by m02_ccode, m02_mkind, m02_yjumin, m02_yname
					 order by m02_yname
					 limit $pageCount, $item_count";
		}

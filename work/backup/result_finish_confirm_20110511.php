		...
		if ($mode == 1){?>
			...
			<tr>
				<td class="my_border_blue my_center" colspan="4">
					<span class="btn_pack m"><button type="button" onclick="_work_confirm('<?=$code;?>','<?=$year;?>','<?=$month;?>','<?=$mode;?>');" style="font-weight:bold;" <? if($data[$month]['conf_flag'] == 'Y'){?>disabled="true"<?} ?>>실적일괄확정 실행</button></span>
				</td>
			</tr><?
		}else{?>
			<tr>
			...
			<tr>
				<td class="my_border_blue my_center" colspan="4">
					<span class="btn_pack m"><button type="button" onclick="_work_confirm('<?=$code;?>','<?=$year;?>','<?=$month;?>','<?=$mode;?>');" style="font-weight:bold;" <? if($data[$month]['calc_flag'] == 'Y' || $data[$month]['conf_flag'] == 'N'){?>disabled="true"<?} ?>>급여일괄계산 실행</button></span>
				</td>
			</tr><?
		}
	?>
...
<?
	if ($mode == 1){
		if ($data[$month]['calc_flag'] == 'Y'){?>
			<div class="noborder my_left my_bold" style="margin-left:10px;">※ <?=$year;?>년 <?=$month;?>월은 <font color="ff0000">급여자동계산</font>이 완료되어 <font color="ff0000">실적일괄확정을</font> 실행할 수 없습니다.</div><?
		}else if ($data[$month]['conf_flag'] == 'Y'){?>
			<div class="noborder my_left my_bold" style="margin-left:10px;">※ <?=$year;?>년 <?=$month;?>월은 <font color="ff0000">실적일괄확정</font>이 이미 완료되었습니다.</div><?
		}
	}else{
		if ($data[$month]['calc_flag'] == 'Y'){?>
			<div class="noborder my_left my_bold" style="margin-left:10px;">※ <?=$year;?>년 <?=$month;?>월은 <font color="ff0000">급여자동계산</font>이 이미 완료되었습니다.</div><?
		}else if ($data[$month]['conf_flag'] == 'N'){?>
			<div class="noborder my_left my_bold" style="margin-left:10px;">※ <?=$year;?>년 <?=$month;?>월의 <font color="ff0000">실적일괄확정</font>이 선행되어야합니다.</div>
			<div class="noborder my_left my_bold" style="margin-left:10px;"><span style="padding-left:17px;"><a href="#" onclick="__go_menu('work','result_finish_confirm.php?mode=1&year=<?=$year;?>&month=<?=$month;?>');" style="font-weight:bold;">[실적일괄확정 바로가기]</a></span></div><?
		}
	}

<?
	...
	// 급여마감일
	$sql = "select salary_cls_dt_from
			  from closing_progress
			 where org_no          = '$code'
			   and closing_yymm like '$year$month'
			   and del_flag        = 'N'";

	if ($conn->get_data($sql) > date('Y-m-d', mktime())){
		$edit_mode = true;
	}else{
		$edit_mode = false;
	}
...
<div style="width:auto; font-weight:bold; margin-top:9px; text-align:right;"><?=!$edit_mode ? '※ 급여가 마감되었습니다.' : '';?></div>

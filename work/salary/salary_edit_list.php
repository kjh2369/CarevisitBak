<?
	...
	$month = $_REQUEST['month'] != '' ? $_REQUEST['month'] : ($max_month > 0 ? $max_month : intval(date('m', mktime())));
		...
		if ($row_count > 0){
			for($i=0; $i<$row_count; $i++){
				...
				<tr>
					...
					<td class="left"		><a href="#" onclick="_salary_edit('<?=$row['kind'];?>','<?=$ed->en($row['jumin']);?>');"><?=$row['name'];?></a></td>

<?
	...
	if ($temp_data[0] == ''){
		// 일괄확정 진행일이 아직 되지 앟았다.
		if ($pos == 1){
			echo '실적 마감일이 등록되지 않았습니다. 실적 마감일을 먼저 등록하여 주십시오.';
		}else{
			echo $myF->message('실적 마감일이 등록되지 않았습니다. 실적 마감일을 먼저 등록하여 주십시오.', 'Y', 'N', 'Y');
		}
		exit;
	}

	if ($temp_data[0].' 18:00:00' > date('Y-m-d H:i:s', mktime())){
		// 일괄확정 진행일이 아직 되지 앟았다.
		if ($pos == 1){
			echo '실적 마감일 이후 일괄확정이 가능합니다. 실적 마감일은 '.$temp_data[0].' 18:00 입니다.';
		}else{
			echo $myF->message('실적 마감일 이후 일괄확정이 가능합니다. 실적 마감일은 '.$temp_data[0].' 18:00  입니다.', 'Y', 'N', 'Y');
		}
		exit;
	}
	...
	if ($temp_data[2] > date('Y-m-d', mktime())){
		// 일괄확정 진행일이 아직 되지 앟았다.
		if ($pos == 1){
	...
	// 수급자
	$sql = "select code, kind, jumin, name, ylvl, skind, bonin_yul, kupyeo_max, min(date_from) as date_from, max(date_to) as date_to, client_key
			  from (
			...
			group by code, kind, jumin, name, ylvl, skind, bonin_yul, kupyeo_max, client_key
			order by jumin, skind, date_from, date_to";

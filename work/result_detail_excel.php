<?
	...
	<table class="my_table" border="1">
		...
		<thead>
			<tr><?
				if($mode == 'DAY_EXCEL'){ ?>
					<th class="head" style="background-color:#eeeeee;">고객명</th><?
				} 
				if($mode != 'DAY_EXCEL'){ ?>
				...
				<?
				if($mode == 'DAY_EXCEL'){ ?>
					<th class="head" style="background-color:#eeeeee;">차이</th>
		...
		<tbody>
			...
			for($i=0; $i<$rowCount; $i++){
				...
				if ($row['request'] != 'LOG'){
					...
					$lsConf = '<span id="lblPlanFrom_'.$i.'">'.$myF->timeStyle($row['conf_from']).'</span>~<span id="lblPlanTo_'.$i.'">'.$myF->timeStyle($row['conf_to']).'</span> (<span id="lblPlanTime_'.$i.'">'.$row['conf_time'].'</span>분)';
				}else{

<?
	...
	if (!is_numeric($_POST["mJumin"])){
		$mJumin = $ed->de($_POST["mJumin"]);
	}else{
		$mJumin = $_POST["mJumin"];
	}
...
// 서비스 제공일정표
function serviceCalendarShow(p_code, p_kind, p_year, p_month, p_target, p_type, p_useType, p_printType){
	var width  = 1000;
	var height = 600;
	var left = (window.screen.width  - width)  / 2;
	var top  = (window.screen.height - height) / 2;
	...
	if (p_printType == 'pdf'){
		window.open('../work/cal_pdf_show.php?code='+p_code+'&kind='+p_kind+'&year='+p_year+'&month='+p_month+'&target='+p_target+'&type='+p_type+'&useType='+p_useType, 'POPUP', 'width=700,height=900,scrollbars=yes,resizable=yes,location=no,toolbar=no,menubar=no');
	}else{
		window.open('../work/cal_show.php?code='+p_code+'&kind='+p_kind+'&year='+p_year+'&month='+p_month+'&sugupja='+p_target+'&type='+p_type+'&useType='+p_useType, 'POPUP', 'left='+left+', top='+top+', width='+width+', height='+height+', toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=no');
	}
}

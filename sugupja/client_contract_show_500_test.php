<?
	...
	$pos[sizeof($pos)] = array('x'=>$pdf->left * 1.15, 'y'=>$pdf->GetY()*1.25, 'font_size'=>11, 'type'=>'multi_text', 'width'=>$pdf->width * 0.1, 'height'=>4.5, 'align'=>'L', 'text'=>"이용자\n  (갑)");
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left * 8.13, 'y'=>$pdf->GetY()*1.02, 'font_size'=>11, 'type'=>'multi_text', 'width'=>$pdf->width * 0.14, 'height'=>4.5, 'align'=>'C', 'text'=>"등  급/\n인정번호");
	
	$lvl = $myF->euckr($myF->_lvlNm($su['level']));


	$lvlNo = $su['level'] != '' ? $lvl."/\n".$su['injungNo'] : '';

	$pos[sizeof($pos)] = array('x'=>$pdf->left * 10.4, 'y'=>$pdf->GetY()*1.02, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width * 0.24, 'height'=>4.5, 'align'=>'C', 'text'=>$lvlNo);
	...
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.001, 'y'=>$pdf->GetY()+5, 'font_size'=>11, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>5, 'align'=>'L', 'text'=>"제1조(목적)", 'font_bold'=>'B');

	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+10, 'font_size'=>8, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>4, 'align'=>'L', 'text'=>stripslashes(str_replace(chr(13).chr(10), "\n", "고령이나 노인성질병 등으로 인하여 혼자서 일상생활을 수행하기 어려운 노인들 중 장기요양등급을 받은 분들에게 방문목욕급여를 제공하여 노후의 건강증진 및 생활안정을 도모하고 그 가족의 부담을 덜어줌으로써 삶의 질을 향상시키고자 한다. ")));
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.001, 'y'=>$pdf->GetY()+19, 'font_size'=>11, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>5, 'align'=>'L', 'text'=>"제2조(계약기간)", 'font_bold'=>'B');

	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+24, 'font_size'=>8, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>4, 'align'=>'L', 'text'=>"① 계약기간은 ".$from_year."년 ".$from_month."월 ".$from_day."일부터 ".$to_year."년 ".$to_month."월 ".$to_day."일까지로 한다.\n② 제1항의 계약기간은 당사자 간의 협의에 따라 변경할 수 있다.");

	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.001, 'y'=>$pdf->GetY()+33, 'font_size'=>11, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>5, 'align'=>'L', 'text'=>"제3조(급여범위)", 'font_bold'=>'B');
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+38, 'font_size'=>8, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>4, 'align'=>'L', 'text'=>stripslashes(str_replace(chr(13).chr(10), "\n", "방문목욕급여는 장기요양요원이 '갑'  의 가정 등을 방문하여 신체활동 및 가사활동 등을 지원하는 장기요양급여로 한다.")));
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.001, 'y'=>$pdf->GetY()+43, 'font_size'=>11, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>5, 'align'=>'L', 'text'=>"제4조(급여이용 및 제공)", 'font_bold'=>'B');
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+48, 'font_size'=>8, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>4, 'align'=>'L', 'text'=>"① 방문목욕급여 이용 및 제공은 장기요양급여 이용(제공)계획서에 의한다.\n② '갑'의 방문목욕급여 이용시간은 아래와 같이 한다. 다만, 방문목욕급여는 주 1회까지 이\n      용이 가능하며 변실금 및 요실금 등으로 인하여 피부의 건강유지관리가 불가피한 경우에는 초과하여 이용할 수 있다.");
	
	$pdf->SetFont($pdf->font_name_kor, "", $pdf->font_size);
	$pdf->SetXY($pdf->left, $pdf->getY()+63);
	...
	$pdf->Cell($pdf->width*0.4, $pdf->row_height, "□매주 □격주 □매월", 1, 1, "C");
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+5, 'font_size'=>8, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>4, 'align'=>'L', 'text'=>"③ '을' 은 '갑' 에게 방문목욕급여를 제공할 때에 반드시 2명의 요양보호사를 배치한다.\n④ '을' 은 익월 장기요양급여 제공을 하고자하는 경우에는 '갑' (또는 '을')과 협의하여 당월  00일까지 별지 제1호서식의 장기요양급여 이용계획서를 작성하고 장기요양급여 이용계획서에 따라 장기요양급여를 제공 한다.");
	...
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$st_getY+91, 'font_size'=>8, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>4, 'align'=>'L', 'text'=>"① '갑'  (또는 '병' )은 다음 각호에 해당되는 경우에는 '을' 과 협의하여 계약을 해지 할 수 있다.
	  ...
	  3. 제4조제2항의 방문목욕급여 제공시간을'갑'  (또는'병' )의 동의 없이 '을' 이 임의로 변경하거나 배치된 장기요양요원을 임의로 변경 했을\n      경우
   ...
   4. 기타 '갑'  과 '을' 이 필요한 경우");
	...
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$st_getY+229, 'font_size'=>8, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>4, 'align'=>'L', 'text'=>" ① '을' 은 방문목욕급여 제공시간에 '갑'  의 생명이 위급한 상태라고 판단된 때에는 '갑'  (또는 '병' )이 지정한 병원 또는 관련 의료기관으로 즉시 후송하고 '병' 에게 즉시 통보하여야 한다.\n ② '병' 은 제1항의 규정에 의한 통보를 받았을 때에는 신속하게 대처하여야 한다. 다만, 대처가 어려울 경우에는 우선 진료를 받을 수 있도록 조치하여야 한다.\n ③ '갑'  이 서비스 이용도중 사망하였을 경우'을' 은 즉시'병' 에게 통보한다.");

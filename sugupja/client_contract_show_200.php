<?
	...
	$to_h1 = $ct['to_time1'] != '' ? substr($ct['to_time1'],0,2) : '     ';
	$to_m1 = $ct['to_time1'] != '' ? substr($ct['to_time1'],2,2) : '     ';
	$fm_h2 = $ct['from_time2'] != '' ? substr($ct['from_time2'],0,2) : '     ';
	$fm_m2 = $ct['from_time2'] != '' ? substr($ct['from_time2'],2,2) : '     ';
	$to_h2 = $ct['to_time2'] != '' ? substr($ct['to_time2'],0,2) : '     ';
	$to_m2 = $ct['to_time2'] != '' ? substr($ct['to_time2'],2,2) : '     ';
	...
	if(str_replace('-','', $from_dt) >= '20180101'){
		...
		if(strlen($center['centerName']) > 22){
			$pos[sizeof($pos)] = array('x'=>$pdf->left * 4.8, 'y'=>$pdf->GetY()*1.01 , 'font_size'=>11, 'type'=>'multi_text', 'width'=>$pdf->width * 0.27, 'height'=>4.5, 'align'=>'L', 'text'=>$center['centerName']);
		}else {
			$pos[sizeof($pos)] = array('x'=>$pdf->left * 4.8, 'y'=>$pdf->GetY()*1.02 , 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width * 0.27, 'height'=>4.5, 'align'=>'C', 'text'=>$center['centerName']);
		}
	...
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+88, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>6, 'align'=>'L', 'text'=>"① 방문요양급여 이용 및 제공은 장기요양급여 이용(제공)계획서에 의한다.\n② '갑'과 '을' 의 방문요양급여 이용시간은 아래와 같이 한다. 다만, 1일 2회 방문요양급여를 제공하는 경우에는 2시간 이상의 간격으로 제공한다.");
	...
	$pos[sizeof($pos)] = array('x'=>$pdf->left, 'y'=>$pdf->GetY()+111+$interval, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width*0.2, 'height'=>6, 'align'=>'C', 'text'=>"이용\n시간\n(1)");

	$pos[sizeof($pos)] = array('x'=>$pdf->left, 'y'=>$pdf->GetY()+135+$interval, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width*0.2, 'height'=>6, 'align'=>'C', 'text'=>"이용\n시간\n(2)");
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left+$pdf->width*0.1, 'y'=>$pdf->GetY()+114.5+$interval, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width*0.6, 'height'=>6, 'align'=>'C', 'text'=>"□월  □화  □수  □목\n □금  □토  □일");
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left+$pdf->width*0.1, 'y'=>$pdf->GetY()+138.5+$interval, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width*0.6, 'height'=>6, 'align'=>'C', 'text'=>"□월  □화  □수  □목\n □금  □토  □일");
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left+$pdf->width*0.5, 'y'=>$pdf->GetY()+116+$interval, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width*0.6, 'height'=>6, 'align'=>'C', 'text'=>$fm_h1."시 ".$fm_m1."분    ~".$to_h1."시 ".$to_m1."분");
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left+$pdf->width*0.5, 'y'=>$pdf->GetY()+140+$interval, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width*0.6, 'height'=>6, 'align'=>'C', 'text'=>$fm_h2."시 ".$fm_m2."분    ~".$to_h2."시 ".$to_m2."분");

	$pdf->SetXY($pdf->left, $pdf->getY()+102+$interval);
	...
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+23, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>6, 'align'=>'L', 'text'=>"③ 관공서의 공휴일에 관한 규정’에 의한 공휴일에 급여를 제공하는 경우에는 '을' 은\n     30%의 할증비용을 청구할 수 있다.\n④ 야간(18:00~22:00), 심야(22:00~06:00)에 급여를 제공하는 경우에는 '을' 은 야간\n     20%, 심야 30%의 할증 비용을 청구할 수 있다.\n⑤ 야간심야휴일가산이 동시에 적용되는 경우에는 중복하여 가산하지 않는다.\n⑥ '을' 은 익월 장기요양급여 제공을 하고자하는 경우에는'갑'  (또는 '병' )과 협의하여\n     급여개시 전까지 급여계획서를 작성하고 수급자(보호자)확인받아 급여서비스를\n     실시한다.");
		//당월 ?일까지 별지 제1호서식의 장기요양급여 이용계획서를 작성하고 장기요양급여 이용\n     계획서에 따라 장기요양급여를 제공 한다.");
	
	$pdf->SetFont($pdf->font_name_kor, "B", $pdf->font_size + 2);
	$pdf->Text($pdf->left,$pdf->getY()+79,'제5조(계약자 의무)');
	
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+81, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>6, 'align'=>'L', 'text'=>"① '갑'  은 다음 각 호를 성실하게 이행하여야 한다.\n   1. 월 이용료 납부\n   2. 방문요양급여 범위내 급여이용\n   3. 장기요양급여 이용수칙 준수\n");
   ...
   4. 기타 '갑'  과 '을' 이 필요한 경우");
	...
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+137, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>6, 'align'=>'L', 'text'=>"① '을' 은 방문요양급여 제공시간에 '갑'  의 생명이 위급한 상태라고 판단된 때에는 '갑'  (\n     또는 '병' )이 지정한 병원 또는 관련 의료기관으로 즉시 후송하고 '병' 에게 즉시 통보\n      하여야 한다.\n② '병' 은 제1항의 규정에 의한 통보를 받았을 때에는 신속하게 대처하여야 한다. 다만, 대처\n      가 어려울 경우에는 우선 진료를 받을 수 있도록 조치하여야 한다.\n③ '갑'  이 서비스 이용도중 사망하였을 경우'을' 은 즉시'병' 에게 통보한다.");
	...
	$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+92, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>6, 'align'=>'L', 'text'=>"① 이 계약서에서 규정하지 않은 사항은 민법이나 사회상규에 따른다.\n② 부득이한 사정으로 소송이 제기될 경우 '갑'  (또는 '병' ) 또는 시설이 속한 소재지역의\n    관할법원으로 한다.");
		...
		/*
		$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+95, 'font_size'=>11, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>6, 'align'=>'L', 'text'=>"※본인은 아래의 자료에 대하여 횡성장기요양센터 관리자로부터  충분한 설명을 들었고 수령하였음을 확인합니다.");
		
		if($code == '34273000017'){
			$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+110, 'font_size'=>10, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>4, 'align'=>'L', 'text'=>"1.장기요양서비스 이용수칙\n2.요양보호사 가능한 업무\n3.요양보호사 불가능한 업무\n4.낙상사고예방법\n5.욕창예방\n6.관절구축예방\n7.배변도움\n8.탈수예방\n9.노인학대 예방 및 노인보호신고기관(전화:1577-1389)\n10.치매예방\n11.기타서류 등");
		}

		if($ct['other_text1'] != ''){
			
			$pdf->SetFont($pdf->font_name_kor, "B", $pdf->font_size + 2);
			$pdf->Text($pdf->left,$pdf->getY()+102,'제16조(별첨사항)');
			...
			$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+155, 'font_size'=>15, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>8, 'align'=>'L', 'font_bold'=>'B', 'text'=>"  위와 같이 계약을 체결하고 본 계약체결을 증명하기 위하여 쌍방이 계약서를 작성 날인 후 각각 1부씩 보관키로 한다.");
			

			$pdf->SetFont($pdf->font_name_kor, "B", $pdf->font_size + 4);
			$pdf->SetXY($pdf->left+45,$pdf->GetY()+175);
			$pdf->Cell($pdf->width*0.2, $pdf->row_height, substr($ct['reg_dt'],0,4)."년", 0, 0, "R");
			$pdf->Cell($pdf->width*0.125, $pdf->row_height, substr($ct['reg_dt'],5,2)."월", 0, 0, "R");
			$pdf->Cell($pdf->width*0.125, $pdf->row_height, (substr($ct['reg_dt'],8,2) < 10 ? substr($ct['reg_dt'],9,1) : substr($ct['reg_dt'],8,2))."일", 0, 1, "R");
			...
			if($jikin != ''){
				...
				$exp = strtolower($exp[sizeof($exp)-1]);
				if($exp != 'bmp'){
					$pdf->Image('../mem_picture/'.$jikin, 177, 210, '20', '20');	//기관 직인
				}
			}
	...
	if($ct['other_text1'] != ''){
		$pdf->SetFont($pdf->font_name_kor, "B", $pdf->font_size + 2);
		$pdf->Text($pdf->left,$pdf->getY()+118,'제16조(별첨사항)');
		
		$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+123, 'font_size'=>12, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>6, 'align'=>'L', 'text'=>"위에 기술되지 않은 특이사항은 #별첨1 에 정의 되어 있다.");

		$pos[sizeof($pos)] = array('x'=>$pdf->left + $pdf->width * 0.005, 'y'=>$pdf->GetY()+131, 'font_size'=>15, 'type'=>'multi_text', 'width'=>$pdf->width, 'height'=>8, 'align'=>'L', 'font_bold'=>'B', 'text'=>"  위와 같이 계약을 체결하고 본 계약체결을 증명하기 위하여 쌍방이 계약서를 작성 날인 후 각각 1부씩 보관키로 한다.");
			
		
		$pdf->SetFont($pdf->font_name_kor, "B", $pdf->font_size + 4);
		$pdf->SetXY($pdf->left+45,$pdf->GetY()+160);
		$pdf->Cell($pdf->width*0.2, $pdf->row_height, substr($ct['reg_dt'],0,4)."년", 0, 0, "R");
		...
		if($jikin != ''){
			...
			$exp = strtolower($exp[sizeof($exp)-1]);
			if($exp != 'bmp'){
				$pdf->Image('../mem_picture/'.$jikin, 176, 190, '20', '20');	//기관 직인
			}
		}
		...
		if($jikin != ''){
			...
			$exp = strtolower($exp[sizeof($exp)-1]);
			if($exp != 'bmp'){
				$pdf->Image('../mem_picture/'.$jikin, 176, 185, '20', '20');	//기관 직인
			}
		}
	...
	for($i=0; $i<$rowCount; $i++){
		...
		$arr[$row['m11_mcode2']]['val'] = $row['m11_suga_value'];
		$arr[$row['m11_mcode2']]['date'] = $row['m11_sdate'];
	...
	*/
	...
	$pdf->Cell($pdf->width, $pdf->row_height * 20 / $pdf->font_size, "방문요양 급여비용 및 본인부담 기준(".$myF->dateStyle($maxPay['1']['date'],'.').")", 0, 1, "C");

	$pdf->SetFont($pdf->font_name_kor, "B", $pdf->font_size + 1.5);
	$pdf->SetXY($pdf->left, $pdf->GetY()+10);
	$pdf->MultiCell($pdf->width*0.9, $pdf->row_height, "○ 방문요양의 1회당 이용시간별 급여비용");
	...
	$pdf->Cell($pdf->width*0.27, $rowH, "구 분", 1, 0, "C" ,true);
	$pdf->Cell($pdf->width*0.23, $rowH, "금액(원)", 1, 0, "C" ,true);
	$pdf->Cell($pdf->width*0.27, $rowH, "구 분", 1, 0, "C", true);
	$pdf->Cell($pdf->width*0.23, $rowH, "금액(원)", 1, 1, "C" ,true);
	
	$pdf->SetFont($pdf->font_name_kor, "", $pdf->font_size);

	$pdf->SetX($pdf->left);
	$pdf->Cell($pdf->width*0.27, $rowH, "30분 이상", 1, 0, "C");
	$pdf->Cell($pdf->width*0.23, $rowH, number_format($arr['CCWS1']['val']), 1, 0, "C");
	$pdf->Cell($pdf->width*0.27, $rowH, "150분 이상", 1, 0, "C");
	$pdf->Cell($pdf->width*0.23, $rowH, number_format($arr['CCWS5']['val']), 1, 1, "C");
	
	$pdf->SetX($pdf->left);
	$pdf->Cell($pdf->width*0.27, $rowH, "60분 이상", 1, 0, "C");
	$pdf->Cell($pdf->width*0.23, $rowH, number_format($arr['CCWS2']['val']), 1, 0, "C");
	$pdf->Cell($pdf->width*0.27, $rowH, "180분 이상", 1, 0, "C");
	$pdf->Cell($pdf->width*0.23, $rowH, number_format($arr['CCWS6']['val']), 1, 1, "C");
	
	$pdf->SetX($pdf->left);
	$pdf->Cell($pdf->width*0.27, $rowH, "90분 이상", 1, 0, "C");
	$pdf->Cell($pdf->width*0.23, $rowH, number_format($arr['CCWS3']['val']), 1, 0, "C");
	$pdf->Cell($pdf->width*0.27, $rowH, "210분 이상", 1, 0, "C");
	$pdf->Cell($pdf->width*0.23, $rowH, number_format($arr['CCWS7']['val']), 1, 1, "C");

	$pdf->SetX($pdf->left);
	$pdf->Cell($pdf->width*0.27, $rowH, "120분 이상", 1, 0, "C");
	$pdf->Cell($pdf->width*0.23, $rowH, number_format($arr['CCWS4']['val']), 1, 0, "C");
	$pdf->Cell($pdf->width*0.27, $rowH, "240분 이상", 1, 0, "C");
	$pdf->Cell($pdf->width*0.23, $rowH, number_format($arr['CCWS8']['val']), 1, 1, "C");
	...
	if(file_exists($file) and is_file($file)){
		$pdf->Image($file, 175, $pdf->getY()+230, '20', '20');	//고객 서명
	}
	...
	$pdf->SetFont($pdf->font_name_kor, "", 12);
	$pdf->Cell($pdf->width * 0.12, $rowH, "주 소:", 0, 0, "C");
	$pdf->Cell($pdf->width * 0.88, $rowH, $juso[0] , 0, 0, "L");

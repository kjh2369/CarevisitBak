<?
	...
	#
		...
		//재가데이타
		if ($IsExpensePeriod){
			//2014.10.15 중복 계산문제로 인하여 "DISTINCT" 추가하고 "b.from_ym, b.to_ym"를 주석처리함.
			$sql = 'SELECT	DISTINCT
							a.org_no AS code
					,		a.kind
					,		a.jumin
					...
					/*,		b.from_ym*/
					/*,		b.to_ym*/
					,		REPLACE(c.from_dt,\'-\',\'\') AS date_from
					,		REPLACE(c.to_dt,\'-\',\'\') AS date_to
					,		IFNULL(d.lvl,\'9\') AS lvl
					,		IFNULL(e.amt,0) AS kupyeo_max
					FROM	(
			...
			if ($isPerson){
				$sql .= '
					WHERE	a.jumin = \''.$calTarget.'\'';
			}

			$sql .= '
					ORDER	BY name, jumin, date_from, date_to';
		}else{
					  ...
					  left join (
						   ...
						   ,      rate
							...
							where org_no = \''.$code.'\'
							  and date_format(from_dt, \'%Y%m\') <= \''.$year.$month.'\'
							  and date_format(to_dt,   \'%Y%m\') >= \''.$year.$month.'\'
							order by jumin, from_dt, to_dt desc
						   ) as kind
		...
		if ($debug){
			$client[0]['code'] = '1234';
			$client[0]['kind'] = '0';
			$client[0]['jumin'] = '5511251021212';
			$client[0]['name'] = '가나다';
			$client[0]['client_key'] = '44';
			$client[0]['lvl'] = '3';
			$client[0]['kupyeo_max'] = '964800';
			$client[0]['date_from'] = '20140901';
			$client[0]['date_to'] = '20140925';

			$client[1]['code'] = '1234';
			$client[1]['kind'] = '0';
			$client[1]['jumin'] = '5511251021212';
			$client[1]['name'] = '가나다';
			$client[1]['client_key'] = '44';
			$client[1]['lvl'] = '3';
			$client[1]['kupyeo_max'] = '964800';
			$client[1]['date_from'] = '20140926';
			$client[1]['date_to'] = '20140930';
		}
		*/
		...
		$sql .= ' order by jumin, from_dt, to_dt';
		...
		for($i=0; $i<$client_count; $i++){
			...
			for($j=0; $j<$row_count; $j++){
				...
				if ($client_kind != $liRate){
					...
					if ($IsExpensePeriod){
						$tmpJuminIdx[$row['t01_jumin']] ++;
						$tmpRate = str_replace('.','',$liRate).'_'.$tmpJuminIdx[$row['t01_jumin']];
					}else{
						$tmpRate = $liRate;
					}

					// 계획 데이타 초기화
					$p[$y] = init_array($row['t01_ccode'], $row['t01_mkind'], $row['t01_jumin'], $year.$month, $tmpRate, '1', $client[$i]['kupyeo_max'], $client[$i]['limitCare'], $client[$i]['limitBath'], $client[$i]['limitNurse']);

					// 실적 데이타 초기화
					$c[$y] = init_array($row['t01_ccode'], $row['t01_mkind'], $row['t01_jumin'], $year.$month, $tmpRate, '2', $client[$i]['kupyeo_max'], $client[$i]['limitCare'], $client[$i]['limitBath'], $client[$i]['limitNurse']);
				...
				switch($row['t01_svc_subcode']){
					...
					case '210':
						...
						if($year < '2019'){
							$svcIndex = '4';
							$p[$k]['dmtaPay'] += $row['t01_suga'] * $liRate / 100;
							$c[$k]['dmtaPay'] += $row['t01_suga'] * $liRate / 100;
						}else {
							$svcIndex = '1';
						}
			...
			if ($row_count > 0){
				...
				for($k=0; $k<$p_count; $k++){
					...
					$p[$k]['biPay1'] += $myF->cutOff($p[$k]['dmtaPay']);
					$p[$k]['bonbuTot1'] += $myF->cutOff($p[$k]['dmtaPay']);
				...
				for($k=0; $k<$c_count; $k++){
					...
					if(strpos($c[$k]['boninAmt1'], '.')==false){
						$c[$k]['boninAmt1'] = $myF->cutOff(ceil(stripslashes($c[$k]['boninAmt1'])));	
					}else {	
						$c[$k]['boninAmt1'] = $myF->cutOff($c[$k]['boninAmt1']);
					}

					
					if(strpos($c[$k]['boninAmt2'], '.')==false){
						$c[$k]['boninAmt2'] = $myF->cutOff(ceil(stripslashes($c[$k]['boninAmt2'])));	
					}else {	
						$c[$k]['boninAmt2'] = $myF->cutOff($c[$k]['boninAmt2']);
					}
					
					if(strpos($c[$k]['boninAmt3'], '.')==false){
						$c[$k]['boninAmt3'] = $myF->cutOff(ceil(stripslashes($c[$k]['boninAmt3'])));	
					}else {	
					...
					$c[$k]['biPay1'] += $myF->cutOff($c[$k]['dmtaPay']);
					$c[$k]['bonbuTot1'] += $myF->cutOff($c[$k]['dmtaPay']);
	...
	# 테이타 저장
		...
		*********************************************************/
		...
		for($i=0; $i<$conf_count; $i++){
			...
			for($j=0; $j<$p_count; $j++){
				$result = set_data($conn, $conf_data[$i]['p'][$j], $debug, $i);
			...
			for($j=0; $j<$c_count; $j++){
				$result = set_data($conn, $conf_data[$i]['c'][$j], $debug, $i);
		...
		******************************************************************/
		...
		if (!empty($sql)) $sql = 'insert into t13sugupja (t13_ccode,t13_mkind,t13_jumin,t13_pay_date, t13_bonin_yul, t13_type,t13_result_amt
	...
	// 데이타 저장
	function set_data($conn, $a, $admin = false, $idx = 0){
		if (!Empty($a['ccode']) &&
			!Empty($a['jumin']) &&
			!Empty($a['payDate'])){

...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
	...
	*/
	...
	//색상
	public function setBGColor($cell, $RGB){
		$this->getStyle($cell)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
		$this->getStyle($cell)->getFill()->getStartColor()->setARGB('00'.$RGB);
	}

	//폰트색상
	public function setFontColor($cell, $RGB){
		$color = new PHPExcel_Style_Color();
		$color->setRGB($RGB);

		$this->getStyle($cell)->getFont()->setColor($color);
	}

	//값
	public function setVal($cellF, $cellT, $val, $style = '', $para = ''){
		if (@$para['V'] == '') $para['V'] = PHPExcel_Style_Alignment::VERTICAL_CENTER;
		if (@$para['H'] == '') $para['H'] = PHPExcel_Style_Alignment::HORIZONTAL_CENTER;

		$this->getCell($cellF)->setValue($val);
		$this->getStyle($cellF)->getAlignment()->setWrapText(true);
		$this->getStyle($cellF.':'.$cellT)->getAlignment()->setVertical($para['V'])->setHorizontal($para['H']);
		$this->mergeCells($cellF.':'.$cellT);

		if ($style) $this->getStyle($cellF.':'.$cellT)->applyFromArray(fnStyle($style));
		if ($para['BackColor']) $this->SetBGColor($cellF,$para['BackColor']);
		if ($para['Format']) $this->getStyle($cellF)->getNumberFormat()->setFormatCode($para['Format']);
	}
	...
	public function SetData($val){
		...
		//$this->getStyle($val['F'])->getAlignment()->setWrapText(false);

		//값입력
		if ($val['format'] == 'text'){
			$val['format'] = '';
			$this->setCellValueExplicit($val['F'], $val['val'], PHPExcel_Cell_DataType::TYPE_STRING);
		}else{
			$this->getCell($val['F'])->setValue($val['val']);
		}

		if ($val['fit']){
			$this->getStyle($val['F'])->getAlignment()->setShrinkToFit(true);
		}else{
		...
		//머지
		if ($val['F'] != $val['T']) $this->mergeCells($val['F'].':'.$val['T']);
		...
		//퍼센트
		if ($val['percent']) $this->getStyle($val['F'])->getNumberFormat()->applyFromArray(array('code'=>PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE_00));

		//글자색
		if ($val['color']) $this->setFontColor($val['F'], $val['color']);

		//폰트 스타일
		if ($val['FontSize']) $this->getStyle($val['F'])->getFont()->setSize($val['FontSize']);
		if ($val['size']) $this->getStyle($val['F'])->getFont()->setSize($val['size']);
		if ($val['bold']) $this->getStyle($val['F'])->getFont()->setBold($val['bold']);

		//들여쓰기
		$this->getStyle($val['F'])->getAlignment()->setIndent($val['indent']);
	}

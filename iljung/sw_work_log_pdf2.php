<?
	...
	$yymm = $var['yymm'];
	$seq = $var['seq'];
	$cnt = str_replace('회차','', iconv('utf-8','euc-kr', $var['cnt']));
	...
	*/
	...
	//질병
	$diseaseStr = "질병명 : ".$pdf->_splitTextWidth($myF->utf(StripSlashes($info['disease'] ? $info['disease'] : "                         ")), $col[2] * 0.4);
	...
	//진단명
	$diseaseStr .= "   진단명 : ".$pdf->_splitTextWidth($myF->utf(StripSlashes($info['diagnosis'])), $col[2] * 0.5)."\n";

	//장애명
	$diseaseStr .= "장애명 : ".$pdf->_splitTextWidth($myF->utf(StripSlashes($info['disabled'])), $col[2] * 0.7);

	if ($row['disabled_lvl'] > 0){
		$diseaseStr .= "(".$info['disabled_lvl']."등급)";
	}
	...
	//의사소통기타
	$commStr .= "기타 : ".$pdf->_splitTextWidth($myF->utf(StripSlashes($info['comm_other'])), $col[2] * 0.9);
	...
	*/
	...
	$pdf->Cell($col[2]/2, 6, $Cnt.'회', 1, 0,'C');
	$pdf->Cell($col[2]/2, 6, $Cnt2.'회', 1, 1,'C');
	...
	if(!empty($row['write_log_yn'])){
		$checkStr .= "근무일지작성 : ".($row['write_log_yn'] == 'Y' ? "Y  " : "N  ");
	}else {
		$checkStr .= "근무일지작성 :     ";
	}
	if(!empty($row['provide_chk_yn'])){
		$checkStr .= "제공시간준수 : ".($row['provide_chk_yn'] == 'Y' ? "Y\n" : "N\n");
	}else {
		$checkStr .= "제공시간준수 :   \n";
	}
	if(!empty($row['right_svc_yn'])){
		$checkStr .= "적절서비스 : ".($row['right_svc_yn'] == 'Y' ? "Y     " : "N     ");
	}else {
		$checkStr .= "적절서비스 :        ";
	}
	if(!empty($row['uniform_yn'])){
		$checkStr .= "유니폼착용 : ".($row['uniform_yn'] == 'Y' ? "Y\n" : "N\n");
	}else {
		$checkStr .= "유니폼착용 :     \n";
	}
	...
	if(!empty($row2['write_log_yn'])){
		$checkStr2 .= "근무일지작성 : ".($row2['write_log_yn'] == 'Y' ? "Y  " : "N  ");
	}else {
		$checkStr2 .= "근무일지작성 :     ";
	}
	if(!empty($row2['provide_chk_yn'])){
		$checkStr2 .= "제공시간준수 : ".($row2['provide_chk_yn'] == 'Y' ? "Y\n" : "N\n");
	}else {
		$checkStr2 .= "제공시간준수 :   \n";
	}
	if(!empty($row2['right_svc_yn'])){
		$checkStr2 .= "적절서비스 : ".($row2['right_svc_yn'] == 'Y' ? "Y     " : "N     ");
	}else {
		$checkStr2 .= "적절서비스 :        ";
	}
	if(!empty($row2['uniform_yn'])){
		$checkStr2 .= "유니폼착용 : ".($row2['uniform_yn'] == 'Y' ? "Y\n" : "N\n");
	}else {
		$checkStr2 .= "유니폼착용 :     \n";
	}
	...
	if ($Y >= $pdf->height - $H[1]){
		...
		$tmpH = ($H[1]+$H[2]+$H[4]+$H[5]+$H[6]+$H[7]+$H[8] - $pdf->_getStrY($pdf,$col[0],"급여 및\n인지활동\n프로그램\n제공확인")) / 2;
		$pdf->SetXY($X, $Y + $tmpH);
		$pdf->MultiCell($col[0], 4, "급여 및\n인지활동\n프로그램\n제공확인", 0, "C");
	}
	...
	if ($Y >= $pdf->height - $H[2]){
		...
		$tmpH = ($H[2]+$H[4]+$H[5]+$H[6]+$H[7]+$H[8] - $pdf->_getStrY($pdf,$col[0],"급여 및\n인지활동\n프로그램\n제공확인")) / 2;
		$pdf->SetXY($X, $Y + $tmpH);
		$pdf->MultiCell($col[0], 4, "급여 및\n인지활동\n프로그램\n제공확인", 0, "C");
	}
	...
	//$pdf->SetXY($X1, $Y);
	//$pdf->Cell($col[2]/2, $H[2], "급여제공 중 방문(".($row['svcporc_yn'] == 'Y' ? ' V ' : '    ').")", 0, 0, "R");

	//$pdf->SetXY($X2, $Y);
	//$pdf->Cell($col[2]/2, $H[2], "급여제공 중 방문(".($row2['svcporc_yn'] == 'Y' ? ' V ' : '    ').")", 0, 0, "R");
	...
	if ($Y >= $pdf->height - $H[4]){
		...
		$tmpH = ($H[4]+$H[5]+$H[6]+$H[7]+$H[8] - $pdf->_getStrY($pdf,$col[0],"급여 및\n인지활동\n프로그램\n제공확인")) / 2;
		$pdf->SetXY($X, $Y + $tmpH);
		$pdf->MultiCell($col[0], 4, "급여 및\n인지활동\n프로그램\n제공확인", 0, "C");
	...
	if ($Y >= $pdf->height - $H[5]){
		...
		$tmpH = ($H[5]+$H[6]+$H[7]+$H[8] - $pdf->_getStrY($pdf,$col[0],"급여 및\n인지활동\n프로그램\n제공확인")) / 2;
		$pdf->SetXY($X, $Y + $tmpH);
		$pdf->MultiCell($col[0], 4, "급여 및\n인지활동\n프로그램\n제공확인", 0, "C");
	}
	...
	//3-4.사회복지사 성명
	$pdf->SetXY($X1, $Y);
	$pdf->Cell($col[1], $H[5], "사회복지사 성명", 1, 0, "C");
	...
	if ($Y >= $pdf->height - $H[6]){
		...
		$pdf->SetFont($pdf->font_name_kor, '', 9);
		$pdf->SetX($X);
		$pdf->Cell($col[0], $H[6]+$H[7], "급여제공확인", 1, 0, "C");
		$Y = $pdf->GetY();

		$tmpH = ($H[6]+$H[7] - $pdf->_getStrY($pdf,$col[0],"급여 및\n인지활동\n프로그램\n제공확인")) / 2;
		$pdf->SetXY($X, $Y + $tmpH);
		$pdf->MultiCell($col[0], 4, "급여 및\n인지활동\n프로그램\n제공확인", 0, "C");
	}
	...
	//3-4.급여제공 중 방문여부
	$pdf->SetXY($X1, $Y);
	$pdf->Cell($col[1], $H[6], "급여제공 중 방문여부", 1, 0, "C");
	...
	if ($Y >= $pdf->height - $H[7]){
		...
		$tmpH = ($H[2] - $pdf->_getStrY($pdf,$col[0],"급여 및\n인지활동\n프로그램\n제공확인")) / 2;
		$pdf->SetXY($X, $Y + $tmpH);
		$pdf->MultiCell($col[0], 4, "급여 및\n인지활동\n프로그램\n제공확인", 0, "C");
	}
	...
	//3-4.방문불가 사유
	$pdf->SetXY($X1, $Y);
	$pdf->Cell($col[1], $H[7], "방문불가 사유", 1, 0, "C");
	$pdf->Cell($col[2]/2, $H[7], "", 1, 0);
	$pdf->Cell($col[2]/2, $H[7], "", 1, 1);

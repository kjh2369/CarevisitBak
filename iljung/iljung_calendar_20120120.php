<?
	...
	*********************************************************/
	...
	$client_date = $conn->client_date($mCode, '', $mJuminNo);
				...
				if (!(($i == 1 && $j < $startWeek) || ($i == $totalWeek && $j > $lastWeek))){
					...
					for($k=0; $k<$row_count; $k++){
						...
						if ($row['t01_sugup_date'] == $iljungDate){
							...
							$rowData[$index]['yoy1']		= $row['t01_mem_cd1'];
							$rowData[$index]['yoy2']		= $row['t01_mem_cd2'];
							...
							$subjectParam = array('svcSubCode'		=> $row['t01_svc_subcode'],
												  ...
												  'yoy1'			=> $row['t01_mem_cd1'],
												  'yoy2'			=> $row['t01_mem_cd2'],
...
<?
	...
	function GetInputString($pGubun, $pDay, $pIndex, $pParam){
		if ($pGubun == 'search' and $pParam['statusGbn'] != '1'){
			...
			$newInputString  = '';
			$newInputString .= '<input name="mKind_'      .$pDay.'_'.$pIndex.'" type="hidden" value="'.$pParam['kind'].'">';
	...
	function GetSubject($conn, $pCenterStartDate, $pConfStartDate, $gubun, $pGubun, $topBorder, $subjectID, $pDay, $pIndex, $pDuplicate, $pSugupja, $pToday, $pIljungDate, $pParam, $dt_from, $dt_to, &$family_time, &$bath_time, &$care_day_cnt){
		...
		if ($gubun == 'Y'){
			...
			if ($pParam["yoy1"] != '') $yoyStat[1] = $conn->get_array("select m02_ygoyong_stat, m02_ytoisail from m02yoyangsa where m02_ccode = '".$pParam["centerCode"]."' and m02_yjumin = '".$pParam["yoy1"]."'");
			if ($pParam["yoy2"] != '') $yoyStat[2] = $conn->get_array("select m02_ygoyong_stat, m02_ytoisail from m02yoyangsa where m02_ccode = '".$pParam["centerCode"]."' and m02_yjumin = '".$pParam["yoy2"]."'");

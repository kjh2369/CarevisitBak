<?
	...
	*********************************************************/
	...
	if (is_array($data)){
		foreach($data as $i => $svc){
			...
			for($day=1; $day<=$lastday; $day++){
				...
				$html .= '<div class=\''.$planIs.'\' style=\'float:left; width:auto; margin:right:3px; padding:0 3px 0 3px; font-weight:bold; '.$style.'\'';
				...
				*********************************************************/
				...
				if (!empty($planIs)){
					...
					/*
					 * 동거여부 상관없이 가족관계를 설정해준다.
					if ($familyYN == 'Y'){
						$familyKind = $arrClientFamily[$svc['mem_cd1']]['kind'];

						if (empty($familyKind)) $familyKind = 'S041';
					...
					if ($svcSubKind == '500') $html .= ' =00 =N'; #목욕 요양2구분값.
					*/
	...
	if ($nextYn == 'Y'){
		...
		$html .= '<script type=\'text/javascript\'>
					window.onload = function(){
						/*setTimeout(\'_iljungCareResultTest()\',1);*/
						/*_iljungSaveCareTest("WIN_'.$winID.'");*/
					}';

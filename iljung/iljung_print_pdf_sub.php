<?
	...
	if ($var['mode'] == '101'){
		...
		foreach($svcGbn as $svcIdx => $svc){
			if ($svc){
				...
				/*		
				if (!Empty($teamCd)){
					$sl .= '	INNER   JOIN    client_his_team as team
								ON	    team.org_no = mst.m03_ccode
								AND     team.jumin = mst.m03_jumin
								AND     team.from_ym <= \''.$var['year'].$var['month'].'\'
								AND     team.to_ym >= \''.$var['year'].$var['month'].'\'
								AND     del_flag = \'N\'
								AND		team.team_cd = \''.$teamCd.'\'';
				}
				*/

				$sl .= ' LEFT   JOIN    mst_jumin as tmp
								ON      tmp.org_no = mst.m03_ccode
								AND     tmp.code   = mst.m03_jumin
		...
		*/
		...
		$sql = 'SELECT CONCAT(jumin,\'_\',svc_cd) AS id
				,      CASE real_ssn WHEN real_ssn THEN real_ssn ELSE jumin end AS jumin
				,      name
				,      svc_cd
				,      app_no
	...
	if ($var['mode'] == '101'){
		...
		$sql = 'SELECT	iljung.jumin
				...
				,		iljung.mem_nm2
				FROM	('.$sl.') AS iljung
				INNER	JOIN	( SELECT MIN(m03_mkind), m03_jumin, m03_name, m03_yoyangsa1_nm
								  FROM   m03sugupja
								  WHERE  m03_ccode = \''.$var['code'].'\'
								  GROUP  BY m03_jumin
								  ) AS mst
						ON		mst.m03_jumin = iljung.jumin';
	...
	if ($var['mode'] == '101'){
		...
		for($i=0; $i<$rowCount; $i++){
			...
			//직원일정 내역
			$arrCaln[$row['mem_cd']][$var['cboSvcYn'] == 'A' ? '0' : $row['svc_cd']][$day][$liSeq] = Array(
					'svcCd'		=>$row['svc_cd']
				,	'subCd'		=>$row['sub_cd']
				,	'name'		=>$row['name']
				,	'procTime'	=>$row['proc_time']
		...
		foreach($arrCaln[$jumin] as $svcCd => $caln){
			...
			if ($var['mode'] == '101'){
				...
				if ($svcCd == '0' || $svcCd == '4'){
					...
					$col['infoWidth'][3]	= $pdf->width*0.33;
				...
				if ($svcCd == '0' || $svcCd == '1' || $svcCd == '2' || $svcCd == '3' || $svcCd == '4' || $svcCd == 'A' || $svcCd == 'B' || $svcCd == 'C'){
					$col['clientValue'][0] = $arrClientInfo[$jumin.'_'.$svcCd]['name'];
				}else if ($svcCd == '6' || $svcCd == 'S' || $svcCd == 'R'){
					$col['clientValue'][0] = $arrClientInfo[$jumin.'_6']['name'];
				}else{
					$col['clientValue'][0] = $arrClientInfo[$jumin.'_'.$svcCd]['name'];
				}
				...
				if ($var['code'] == '31135000055'){ //수락재가
					...
					if ($svcCd == '0' || $svcCd == '1' || $svcCd == '2' || $svcCd == '3' || $svcCd == '4' || $svcCd == 'A' || $svcCd == 'B' || $svcCd == 'C'){
						...
						if(strlen($arrClientInfo[$jumin.'_6']['jumin']) == 13){
							$col['clientValue'][1] = $myF->issStyle($arrClientInfo[$jumin.'_6']['jumin']);	
						}else {
							$len = 13 - intval(strlen($arrClientInfo[$jumin.'_6']['jumin']));
							
							for($i=0; $i<$len; $i++){
								$num .= '0';
							}
							
							$col['clientValue'][1] = $myF->issStyle($arrClientInfo[$jumin.'_6']['jumin'].$num);
				...
				if ($svcCd == '0'){
					...
					$col['clientValue'][3] = $myF->splits($arrClientInfo[$jumin.'_6']['juso'],20);
				}else{
					$col['clientValue'][3] = "";
					$col['clientValue'][4] = "";
				}
							...
							if (Is_Array($arrCaln[$jumin][$svcCd][$day])){
								foreach($arrCaln[$jumin][$svcCd][$day] as $seq => $caln){
									...
									if ($caln['svcCd'] == '6' || $caln['svcCd'] == 'S' || $caln['svcCd'] == 'R'){
										...
										if ($var['mode'] == '101'){
											if ($caln['subCd'] == '200' || $caln['subCd'] == '500'){
												if($caln['familyYn']=='Y'){
													if($caln['subCd'] == '200' && $caln['procTime']==89){
														$caln['procTime'] = 60;
													}
												}
									...
									if ($caln['svcCd'] == 'S' || $caln['svcCd'] == 'R'){
										$pdf->Cell($col['calnWidth'][$j]*0.5, $liHg, $caln['workTime'], 0, 0, 'L');
										$pdf->Cell($col['calnWidth'][$j]*0.5, $liHg, $caln['worker'], 0, 2, 'R');
										$pdf->SetX($pdf->GetX()-$col['calnWidth'][$j]*0.5);
									}else{
									...
									if ($var['mode'] == '101'){
										//고객명
										if ($caln['svcCd'] == 'S' || $caln['svcCd'] == 'R'){
											$pdf->Cell($col['calnWidth'][$j], $liHg, $pdf->_splitTextWidth($myF->utf($caln['resource']),$col['calnWidth'][$j]), 0, 2, 'R');
										}else{
			...
			//제공서비스
			if ($chkSvcCd != '5') lfGetSvcTitle($pdf, $col);
			...
			if ($var['mode'] == '101'){
				if ($pdf->svcCd == '4'){
					...
					//총주사람인에서 기관장을 전담인력으로 수정요청함. 연금공단에서 지적하였다고함.
					
					$pdf->SetXY($pdf->left + $pdf->width * 0.5, $pdf->GetY() + 2);
					$pdf->Cell($pdf->width * 0.5, $pdf->row_height, '전담인력 : '.$arrMst['name'].'                            ', 0, 1, 'R');
					$pdf->SetXY($pdf->left + $pdf->width * 0.5, $pdf->GetY() + 2);
					...
					if($var['code'] == '22820000063'){ //남동엔젤스
						$pdf->SetXY($pdf->left, $pdf->GetY() + 5);
						$pdf->SetFont($pdf->font_name_kor,'B',11);
						$pdf->Cell($pdf->width, $pdf->row_height, '※ 본인부담금을 당월 10일까지 입금부탁드립니다.', 0, 1, 'L');
						$pdf->SetFont($pdf->font_name_kor,'',8);
					}
				}
				...
				if ($chkSvcCd == '5'){
					$pdf->SetXY($pdf->left,$pdf->GetY()+5);
					$pdf->SetFont($pdf->font_name_kor,'',11);
					$pdf->MultiCell($pdf->width, 4, $danMemo);
					$pdf->SetFont($pdf->font_name_kor,'',8);
				}
			}
		}

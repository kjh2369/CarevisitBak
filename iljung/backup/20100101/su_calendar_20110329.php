					...
					for($k=0; $k<$row_count; $k++){
						...
						if ($row['t01_sugup_date'] == $iljungDate){
							...
							$inputParam = array('centerStartDate' => $centerStartDate,
												...
												'holiday'    => $holiday,
												'oldDate'    => $row['t01_sugup_date'].$row['t01_sugup_fmtime'].$row['t01_sugup_totime'],
							...
							for($t=1; $t<=5; $t++){
								if ($_PARAM['yoy'.$t] != ''){
									...
									$sql = 'select count(*)'
										 . '  from t01iljung'
										 . ' where t01_ccode           = \''.$mCode.'\''
										 . '   and t01_sugup_date      = \''.$iljungDate.'\''
										 . '   and (t01_yoyangsa_id1   = \''.$_PARAM['yoy'.$t].'\''
										 . '    or  t01_yoyangsa_id2   = \''.$_PARAM['yoy'.$t].'\''
										 . '    or  t01_yoyangsa_id3   = \''.$_PARAM['yoy'.$t].'\''
										 . '    or  t01_yoyangsa_id4   = \''.$_PARAM['yoy'.$t].'\''
										 . '    or  t01_yoyangsa_id5   = \''.$_PARAM['yoy'.$t].'\')'
									     . '   and ((t01_sugup_fmtime >= \''.$_PARAM["fmTime"].'\' and t01_sugup_fmtime <  \''.$_PARAM["ttTime"].'\')'
										 . '    or  (t01_sugup_totime >  \''.$_PARAM["fmTime"].'\' and t01_sugup_totime <= \''.$_PARAM["ttTime"].'\')'
										 . '    or  (\''.$_PARAM["fmTime"].'\' = \''.$_PARAM["ttTime"].'\' and t01_sugup_fmtime >= \''.$_PARAM["fmTime"].'\'))'
										 . '   and t01_del_yn = \'N\'';
									*/
									/*
									$sql = 'select count(*)'
										 ...
										 . '   and t01_sugup_totime >  \''.$_PARAM["ttTime"].'\''
										 . '   and t01_del_yn = \'N\'';
									*/
									...
									*/
									...
									$sql = "select count(*)"
										 . "  from t01iljung"
										 . " where t01_sugup_date = '".$iljungDate
										 . "'  and '".$_PARAM['yoy'.$t]."' in (t01_yoyangsa_id1, t01_yoyangsa_id2, t01_yoyangsa_id3, t01_yoyangsa_id4, t01_yoyangsa_id5)"
										 . "   and ((t01_sugup_fmtime <= '".$_PARAM['fmTime']."'"
										 . "   and   case when t01_sugup_fmtime > t01_sugup_totime then cast(t01_sugup_totime as unsigned) + 2400 else t01_sugup_totime end > '".$_PARAM['fmTime']."')"
										 . "    or  (t01_sugup_fmtime <  '".$ToTime."'"
								...
								if (sizeOf($rowData) > 0){
									if ($rowData[$k]['date'] == $iljungDate){
										/*
										if ($rowData[$k]['yoy'.$t] != '' and $_PARAM['yoy'.$t] != ''){
											if ($rowData[$k]['yoy'.$t] == $_PARAM['yoy'.$t]){
												if (($rowData[$k]['FmTime'] >= $_PARAM["fmTime"] and $rowData[$k]['FmTime'] <  $_PARAM["ttTime"]) or
													($rowData[$k]['ToTime'] >  $_PARAM["fmTime"] and $rowData[$k]['ToTime'] <= $_PARAM["ttTime"])){
													$duplicate = 'Y';
													break;
												}
											}
										...
										*/
										...
										//if (($rowData[$k]['FmTime'] >= $_PARAM["fmTime"] && $rowData[$k]['ToTime'] > $_PARAM["fmTime"]) or
										//	($rowData[$k]['FmTime'] >= $_PARAM["ttTime"] && $rowData[$k]['ToTime'] > $_PARAM["ttTime"])){
											$duplicate = 'Y';
											break;
										}
									}

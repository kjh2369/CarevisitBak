				...
				if (!(($i == 1 && $j < $startWeek) || ($i == $totalWeek && $j > $lastWeek))){
					...
					for($k=0; $k<$row_count; $k++){
						...
						if ($row['t01_sugup_date'] == $iljungDate){
							...
							$subjectParam = array('svcSubCode'=> $row['t01_svc_subcode'],
												  ...
												  'addAmt'    => 0,
												  'clientLvl' => '');
							$subjectTemp = GetSubject($conn, $centerStartDate, $confStartDate, 'Y', $gubun, $subjectPrint == '' ? '0' : '1', $subjectID, $day, $index, 'N', 'N', $today, $iljungDate, $subjectParam, $dt_from, $dt_to);
					...
					if (($centerStartDate == subStr($iljungDate, 0, 6)) or
						...
						*/
								...
								if (sizeOf($rowData) > 0){
									if ($rowData[$k]['date'] == $iljungDate){
										...
										if ($_PARAM['svcSubCode'] == '200' && $rowData[$k]['svcSubCode'] == '200'){
											if ($duplicate != 'Y'){
												// 전 일정과 2시간 차이를 확인한다.
												$tmpCheckToTime = intval(substr($rowData[$k]['ToTime'],0,2)) * 60 + intval(substr($rowData[$k]['ToTime'],2,2)) + __CHECK_ADD_TIME__;
												$newCheckFmTIme = intval(substr($_PARAM["fmTime"],0,2)) * 60 + intval(substr($_PARAM["fmTime"],2,2));

												if ($tmpCheckToTime > $newCheckFmTIme){
													$duplicate = 'O';
												}
											}
										}
						...
						$subjectTemp = GetSubject($conn, $centerStartDate, $confStartDate, $subject, $gubun, $subjectPrint == '' ? '0' : '1', $subjectID, $day, $index, $duplicate, $sugupja, $today, $iljungDate, $subjectParam, $dt_from, $dt_to);
...
<?
	...
	function GetSubject($conn, $pCenterStartDate, $pConfStartDate, $gubun, $pGubun, $topBorder, $subjectID, $pDay, $pIndex, $pDuplicate, $pSugupja, $pToday, $pIljungDate, $pParam, $dt_from, $dt_to){
		$fontColor = '#000000';
		$showBtn = 'Y';

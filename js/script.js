...
//텍스트 크기 변경
var TextSizeTgs = new Array( 'div','th','td','tr');
var TextSizeSzs = new Array( 'xx-small','x-small','small','medium','large','x-large','xx-large' );
var TextSizeStartSz = 2;
...
function __TextSize(trgt, inc){
	//<a href="javascript:__TextSize('body',1)">+ Larger Font</a> | <a href="javascript:__TextSize('body',-1)">+ Smaller Font</a>
	if (!document.getElementById) return
	var d = document,cEl = null,sz = TextSizeStartSz,i,j,cTags;
	...
	cEl.style.fontSize = TextSizeSzs[ sz ];

	for ( i = 0 ; i < TextSizeTgs.length ; i++ ) {
		cTags = cEl.getElementsByTagName( TextSizeTgs[ i ] );
		for ( j = 0 ; j < cTags.length ; j++ ) cTags[ j ].style.fontSize = TextSizeSzs[ sz ];
	}
}
...
function __DateDiff(date1,date2){
	...
	var d1 = new Date(v1[0],v1[1]-1,v1[2], 0, 0, 0);
	var d2 = new Date(v3[0],v3[1]-1,v3[2], 0, 0, 0);
...
function __getSsn(target, alt){
	...
	if(alt == 'ssn'){
		if(target.value != '' && target.value.length < 7){
			alert('주민번호 7자리를 입력하여주십시오.');
			target.focus();
			return;
		}
	}
...
/* 전화번호 */
function __getPhoneNo(target,gbn){
	if (!gbn) gbn = '-';

	if (typeof(target) == 'object'){
		target.value = target.value.replace(/(^02.{0}|^01.{1}|[0-9]{3})([0-9]+)([0-9]{4})/,'$1'+gbn+'$2'+gbn+'$3');
	}else{
		return target.replace(/(^02.{0}|^01.{1}|[0-9]{3})([0-9]+)([0-9]{4})/,'$1'+gbn+'$2'+gbn+'$3');
	}
}

function __getDate(target,gbn){
	var ret = null;
	var varDate;
	var y, m, d;
	var ry, rm, rd;
	...
	varDate = varDate.split('-').join(gbn);
...
function __isValDate(iDate) {
	...
	oDate.setMonth(parseInt(iDate.substring(4, 6)- 1) ); //월셋팅을 두번쓰지않으면 원치않는값을 불러오는경우가 있음.
	oDate.setDate(iDate.substring(6));
	
	//oDate.setFullYear(iDate.substring(0, 4),parseInt(iDate.substring(4, 6)) - 1,iDate.substring(6));
...
function __addDate(pInterval, pAddVal, pYyyymmdd, pDelimiter){
	var yyyy;
	var mm;
	var dd;
	var cDate;
...
function __alert(p_target){
	...
	case 'date':
		...
		if (!__isValDate(target.value)) result = false;
		break;
	default:
...
// 수급자 찾기
function __find_client(code, target, dt){
	
	var modal = showModalDialog('../inc/_find_person.php?type=sugupja&code='+code+'&kind='+'&regDt='+dt, window, 'dialogWidth:600px; dialogHeight:400px; dialogHide:yes; scroll:no; status:yes');
...
function __checkImageExp2(p_object, abHiddenMsg){
	var object = __getObject(p_object);
	...
	if (object.value != ''){
		...
		if (exp[exp.length-1].toLowerCase() == 'jpg' || exp[exp.length-1].toLowerCase() == 'gif'){
			return true;
		}else{
			if (!abHiddenMsg){
				alert('jpg, gif 의 이미지 파일을 선택하여 주십시오.');
			}
			return false;
		}
	}
...
// 사례관리 사진, 약도 
function __checkImageExp3(p_object, abHiddenMsg){
	var object = __getObject(p_object);
	...
	if (object.value != ''){
		...
		if (exp[exp.length-1].toLowerCase() == 'jpg' || exp[exp.length-1].toLowerCase() == 'gif' || exp[exp.length-1].toLowerCase() == 'png' ){
			return true;
		}else{
			if (!abHiddenMsg){
				alert('jpg, gif 의 이미지 파일을 선택하여 주십시오.');
			...
			if (c_name == 'phone'){
				...
				el.onblur = function(){
					if (alt == 'time'){
						...
						if (this.value.split(':').join('').length != 6){
							this.value = '';
						}else{
							this.value = __styleTime(this.value);
						}
...
function __set_lastday(target){
	try{
		...
		var last = getDay(__addDate('d', -1, __addDate('m', 1, date)));
...
function __go_menu(menu, uri){
	...
	if (typeof(uri) != 'string'){
		if (menu == ''){
			...
			location.href = '../center/list.php?menu='+menu;
		}else if (menu == 'iljung'){
			location.href = '../iljung/iljung_list.php?mode=1&menu='+menu;
		}else if (menu == 'work'){
			location.href = '../iljung/iljung_result.php?gubun=client&menu='+menu;
		}else if (menu == 'claim'){
			location.href = '../claim/public_amt_list.php?menu='+menu;
		}else if (menu == 'salary'){
			location.href = '../salaryNew/salary_edit_list.php?menu='+menu;
		}else if (menu == 'account'){
			location.href = '../account/income_list.php?io_type=i&mode=1&menu='+menu;
		}else if (menu == 'other'){
			location.href = '../goodeos/board_list.php?board_type=noti&menu='+menu;
		}else if (menu == 'report'){
			location.href = '../reportMenu/report.php?report_menu=10&menu='+menu;
		}else if (menu == 'help'){
			location.href = '../help/help.php?menu='+menu;
		}else if (menu == 'branch'){
			location.href='../branch/branch_list.php?mode=branch&menu='+menu;
		}else if (menu == 'goodeos'){
			location.href='../goodeos/notice_list.php?menu='+menu;
		}else if (menu == 'proc'){
			location.href='../work/work_real.php?menu='+menu;
		}else if (menu == 'store'){
			location.href='../branch/branch_list.php?mode=store&menu='+menu;
		}else if (menu == 'company'){
			location.href='../branch/branch_reg.php?mode=company&menu='+menu;
		}else if (menu == 'eval_data'){
			location.href='../eval_data/eval_data.php?menu='+menu;
		}else if (menu == 'GoodEOSAcct'){
			location.href='../acct/acct.php?type=31&menu='+menu;
		}else if (menu == 'care'){
			location.href='../care/care.php?type=1&menu='+menu;
		}else if (menu == 'case'){
			location.href='../hce/hce.php?type=1&menu='+menu;
		}else if (menu == 'report2014'){
			location.href='../report2014/index.php?report_menu=10&menuTopId=H&menu='+menu;
		}
	}else{
		location.href = uri;
	}
...
// yyyy-mm 형식으로 리턴한다.
function __get_yymm(target, gbn){
	var result = '';
...
function __get_loading(){
	return '<center><div class=\'ly_loading\' style=\'\'><p class=\'dsc_loading\'>문서를 불러오는 중...</p><img alt=로딩중 src=\'../common/img/load_b01_02.gif\' width=\'200\' height=\'20\'>';
}
...
*********************************************************/
function __findMember(code,kind,tmpWin){
	if (!tmpWin) tmpWin = window;

	var result = showModalDialog('../inc/_find_person.php?type=member&code='+code+'&kind='+kind, tmpWin, 'dialogWidth:600px; dialogHeight:400px; dialogHide:yes; scroll:no; status:yes');
...
*********************************************************/
function __findTeam(code,kind,tmpWin){
	if (!tmpWin) tmpWin = window;

	var result = showModalDialog('../inc/_find_person.php?type=team&code='+code+'&kind='+kind, tmpWin, 'dialogWidth:600px; dialogHeight:400px; dialogHide:yes; scroll:no; status:yes');
 ...
 ******************************************/
function __ExcelMaker(para){
	var parm = __parseVal(para);
	...
	form.setAttribute('action', '../showExcel/');
...
*********************************************************/
function __svcNm(asSvcCd){
	var svcNm	= '';

	switch(asSvcCd){
		case '0':	svcNm	= '재가요양';		break;
		case '1':	svcNm	= '가사간병';		break;
		case '2':	svcNm	= '노인돌봄';		break;
		case '3':	svcNm	= '산모신생아';		break;
		case '4':	svcNm	= '자애인활동지원';	break;
		case 'A':	svcNm	= '산모유료';		break;
		case 'B':	svcNm	= '병원간병';		break;
		case 'C':	svcNm	= '기타비급여';		break;
	}
...
function __getDt(asFrom, asTo){
	...
	if (asFrom) fromDt = __getDate(asFrom);
	if (asTo) toDt = __getDate(asTo);
 ...
 *	이미지 로드
 *********************************************************/
function __loadImg(obj,width,height){
	try{
		var w = obj.width;
		var h = obj.height;
		var r = 1;
...
/*********************************************************
	년월이동
 *********************************************************/
function __moveYear(obj,pos,fun){
	var year = __str2num($(obj).text());

	year += pos;

	$(obj).attr('year',year).text(year);
...
function __moveMonth(mon,obj,fun){
	$('div[id^="btnMonth_"]').each(function(){
		var m = $(this).attr('id').split('btnMonth_').join('');
 ...
 *	일자버튼
 */
function __btnDay(year,month,day){
	year = __str2num(year);
	month = __str2num(month);
	day = __str2num(day);

	var days = new Array(0,31,28,31,30,31,30,31,31,30,31,30,31);
	var lastday = days[month];
	...
	var html = '<div style="clear:both;">'
			 + '<div id="btnDay_A" class="my_box my_box_1" style="float:left; margin-left:2px;" style="cursor:pointer;" onclick="__moveDay(0);">A</div>';
	...
	for(var i=1; i<=lastday; i++){
		...
		html += '<div id="btnDay_'+i+'" class="my_box my_box_1" style="float:left; margin-left:2px;" style="cursor:pointer; color:'+color+';" onclick="__moveDay('+i+');">'+i+'</div>';
	}
...
function __GetHeight(obj){
	var top = $(obj).offset().top;
	var body = document.body;
	var height = body.offsetHeight - top - 1;

	//return $(document).height() - $(obj).offset().top - 1;
	return height;
}
...
//해당 태그 객체
function __GetTagObject(obj,tag){
	var o = obj;

	while(true){
		if ($(o).attr('tagName') == tag){
			break;
		}
...
//템프로딩바
function __TempLoading(){
	$('#divTemp').after('<div id=\'tempLodingBar\' style=\'position:absolute; width:auto; top:270px; left:470; text-align:center; padding:20px; background-color:#ffffff; border:3px solid #cccccc;\'>'+__get_loading()+'</div></center></div>').show();

	setTimeout('TempLoadingSub()', 500);
}

function TempLoadingSub(){
	$.post('../inc/__temp_load.php',{}, function(html){
		$('#tempLodingBar').remove();
...
//서명관리
function __SetSign(obj, key, gbn){
	...
	if (!objModal.p) return;

	if (obj){
		if ($(obj).attr("writeStyle") == "STRING"){
			$(obj).text("*Sign이 작성되었습니다.");
		}else{
			var pos = __ResizeImg($(obj).width() * 0.9, $(obj).height() * 0.9, __str2num(objModal.w), __str2num(objModal.h));
...
//이미지 사이즈
function __ResizeImg(maxW, maxH, objW, objH){
	var w1 = maxW, h1 = maxH,
...
//해당 태그 객체
function __tagObject(obj,tag){
	var o = obj;

	while(true){
		if ($(o).prop('tagName') == tag){
			break;
		}
...
function __ObjData(obj){
	var data = {};
	
	if (!obj) obj = "input:text, input:checkbox, input:radio, input:hidden, select, textarea";
	...
	$(obj).each(function(){
		...
		if (!$(this).prop("disabled")){
			if ($(this).prop("type") == "checkbox"){
				if ($(this).attr('paraFlag') == 'Y'){
					data[id] = $(this).val()+'&chkYn='+($(this).prop("checked") ? 'Y' : 'N');
				}else{
					data[id] = $(this).prop("checked") ? $(this).val() : "";
				}
			}else if ($(this).prop("type") == "radio"){
				data[$(this).prop("name")] = $("input:radio[name=\""+$(this).prop("name")+"\"]:checked").val();

				if (!data[$(this).prop("name")]) data[$(this).prop("name")] = '';
			}else{
				var val = $(this).val();

				if ($(this).hasClass("date") || $(this).hasClass("phone") || $(this).hasClass("ssn") || $(this).hasClass("yymm") || $(this).hasClass("taxid")){
					val = val.split("-").join("");

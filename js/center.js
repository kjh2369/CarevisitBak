...
function _yoyangsaSave(){
	...
	// 친족케어유무
	if (document.getElementById('yFamCareType').value == 'Y'){
...
function _csvUpload(form, file, action){
	if (file.value == ''){
		alert('업로드할 CSV 파일을 선택하여 주십시오.');
		return;
	}

	var exp = file.value.split('.');
	
	if (exp[exp.length-1].toLowerCase() != 'csv'){
		alert('csv 파일 형식의 파일만 업로드 가능합니다. 확인하여 주십시오.');
		return;
	}

	if (!confirm('선택하신 파일의 데이타를 저장하시겠습니까?')){
		return;
	}
...
function _insToDate(p_fromDate, p_toDate){
	...
	if (checkDate(formDate)){
		toDate.value = __addDate('yyyy', 1, formDate);
	}else{
		toDate.value = '';
	}
...
function _familyCareInsYN(){
	...
	var yFamCareType = document.getElementById('yFamCareType');
	...
	if (yFamCareUmu.value == 'Y'){
		...
		if (yFamCareType.value == '2'){
			yFamCarePay.onkeydown = function(){
				__onlyNumber(this,'.');
			}
			yFamCarePay.onblur = function(){
				if (this.value==''){
					this.value='0.0';
				}
			}
...
function _family_care_yn(){
	...
	if (yn == 'Y'){
		...
		type.disabled = false;
		_family_care_pay_type(type.value);
	}else{
		pay.readOnly = true;
		pay.value = '0';
...
function _family_care_pay_type(type){
	var oliginal_type = document.getElementById('yFamCareType').tag;
	var pay  = __getObject('yFamCarePay');

	if (oliginal_type == type){
		pay.value = pay.tag;
	}else{
		pay.value = 0;
	}
	...
	if (type == '2'){
		pay.setAttribute('maxLength', '4');
		pay.onchange = function(){
			if (parseFloat(this.value) < 0 || parseFloat(this.value) > 100){
				alert('동거가족 비율은 0~100%까지 입력가능합니다. 확인하여 주십시오.');
				this.value = 0;
				return false;
			}
		}

		pay.onkeydown = function(){
			__onlyNumber(this,'.');
		}
		pay.onblur = function(){
			if (this.value == ''){
...
function _checkInsLimitDate(){
	...
	*/
	...
	var ins_yn  = __object_get_value(document.getElementById('ins_yn'));
	var from_dt = document.getElementById('ins_from_dt');
	var to_dt   = document.getElementById('ins_to_dt');
	var c_from  = document.getElementById('centerInsFromDate');
	var c_to    = document.getElementById('centerInsToDate');
	var join_dt = document.getElementById('join_dt');

	if (ins_yn = 'Y'){
		if (__replace(from_dt.value, '-', '') >= __replace(join_dt.value, '-', '')){
			/*
			if (__replace(from_dt.tag, '-', '') >= __replace(join_dt.value, '-', '')){
				from_dt.value = from_dt.tag;
			}else{
				from_dt.value = join_dt.value;
			}
			*/
		}else{
			alert('입사일자 이전의 일자는 입력할 수 없습니다. 입사일자로 입력됩니다.');
			from_dt.value = join_dt.value;
		}

		to_dt.value = to_dt.tag;
...
function _save_center(){
	...
	if (host == 'www'){
		...
		if (f.kind_1.checked){
			if (!_chk_time_pay(f.day_work_hour0, f.day_hourly0)) return;
		}

		if (f.kind_2.checked){
			if (f.kind_2_1.checked){
				if (!_chk_time_pay(f.day_work_hour1, f.day_hourly1)) return;
			}

			if (f.kind_2_2.checked){
				if (!_chk_time_pay(f.day_work_hour2, f.day_hourly2)) return;
			}

			if (f.kind_2_3.checked){
				if (!_chk_time_pay(f.day_work_hour3, f.day_hourly3)) return;
			}

			if (f.kind_2_4.checked){
				if (!_chk_time_pay(f.day_work_hour4, f.day_hourly4)) return;
			}
		}
		...
		if (!__checkImageExp(f.icon)) return;
		*/
	}else{
		if (!__alert(f.mCode)) return;
		if (!__alert(f.storeNm)) return;
	}
...
function _center_save(){
	...
	if (host == 'www'){
		...
		if (!_chk_time_pay(f.day_work_hour, f.day_hourly)) return;
		
		// 요양기관구분 선택확인
		if ((f.kind_cnt_0.value == 0 && !f.kind_1.checked) && 
			(f.kind_cnt_1.value == 0 && !f.kind_2_1.checked) && 
			(f.kind_cnt_2.value == 0 && !f.kind_2_2.checked) && 
			(f.kind_cnt_3.value == 0 && !f.kind_2_3.checked) && 
			(f.kind_cnt_4.value == 0 && !f.kind_2_4.checked) && 
			(f.kind_cnt_5.value == 0 && !f.kind_3.checked)){
			alert('요양기관구분을 하나이상 선택하여 주십시오.');
			return;
		}
	...
	f.action = 'center_reg_save.php';
	f.submit();
}

function _chk_time_pay(day_work_hour, day_hourly){
	// 일근무기준시간
	if (parseFloat(day_work_hour.value, 10) < 2.5){
		alert('일근무기준시간을 입력하여 주십시오. 최소 시간은 2.5시간입니다.');
		day_work_hour.focus();
		return false;
	}

	// 일근무기준급
	if (__str2num(day_hourly.value, 10) < __str2num(day_hourly.tag)){
		alert('일근무기준시급을 입력하여 주십시오. 최소 시급은 '+__num2str(day_hourly.tag)+'입니다.');
		day_hourly.focus();
		return false;
	}
...
// 직원수정
function _mem_reg(code, kind, jumin){
	var f = document.f;

	f.code.value  = code;
	f.kind.value  = kind;
	f.jumin.value = jumin;
	f.action = 'mem_reg.php';
	f.submit();
}
...
/*
2011-04-19 오후8:00 
일반수급케어와 동거가족케어 둘 중 하나도 입력 안됬을 시 에러메시지 띄우기 추가!
*/
function _save_menber_0(f){
	...
	if (__get_value(f.yGoyongStat) == '9'){
		...
		if (ins_yn == 'Y'){
			...
			if (f.yIpsail.value > f.insFromDate.value){
	...
	var yJikJong = document.getElementById("yJikJong");

	if(yJikJong.options[yJikJong.selectedIndex].tag == 'Y'){
		if(document.f.yGupyeoKind[0].checked == true && document.f.yFamCareType[0].checked == true){
			alert('급여산정방식 중 하나는 입력해 주셔야 합니다. 확인하여 주십시오.');
			return;
		}
	}
	...
	/*
	if(document.f.yGupyeoKind[0].checked == true && document.f.yFamCareType[0].checked == true){
		alert('일반수급자케어 또는 동거가족케어 중 하나는 입력해 주셔야 합니다. 확인하여 주십시오.');
		return;
	}
	*/

	if (f.yFamCareType[1].checked == true){
		if (f.yFamCarePay1.value <= 0){
			alert('시급(동거가족)을 입력하여 주십시오.');
			f.yFamCarePay1.focus();
			return false;
		}
	}else if (f.yFamCareType[2].checked == true){
		if (parseFloat(__commaUnset(f.yFamCarePay2.value), 10) < 1 || 
			parseFloat(__commaUnset(f.yFamCarePay2.value), 10) > 100){
			alert('동거가족 비율은 1~100%까지 입력가능합니다. 확인하여 주십시오.');
			f.yFamCarePay2.focus();
			return false;
		}
	}else if (f.yFamCareType[3].checked == true){
		if (f.yFamCarePay3.value <= 0){
			alert('월급(동거가족)을 입력하여 주십시오.');
			f.yFamCarePay3.focus();
			return false;
		}
	}
		...
		if (checkDate(f.insToDate.value)){
			if (__get_value(f.yGoyongStat) == '9'){
				if (f.insToDate.value > f.yToisail.value){
					alert('배상책임보험의 해지일자가 퇴사일자보다 큽니다. 확인하여 주십시오.');
					f.insToDate.focus();
					return false;
				}
			}else{
				if (f.insFromDate.value >= f.insToDate.value){
					alert('배상책임보험의 해지일자가 가입일자보다 큽니다. 확인하여 주십시오.');
					f.insToDate.focus();
					return false;
				}
			}

			if (f.insToDate.value > f.centerInsToDate.value){
				alert('배상책임보험의 해지일자가 기관의 배상책임보험 종료일자보다 큽니다. 확인하여 주십시오.');
				f.insToDate.focus();
				return false;
			}
		}
...
function _ins_join_yn(ins_yn, now_yn, f_date, t_date){
	...
	__setEnabled(f_dtae, (ins_yn == 'Y' ? true : false));
	__setEnabled(t_date, (ins_yn == 'Y' ? true : false));
	
	return;

	var join_date = document.getElementById('yIpsail');

	if (join_date == null){
		join_date = document.getElementById('join_dt');
	}
	...
	if (now_yn == 'N'){
		if (ins_yn == 'Y'){
			...
			if (join_date.value != ''){
				if (join_date.value > f_dtae.tag){
					f_dtae.value = join_date.value;
				}else{
...
function _list_client(page){
	...
	//__setCookie('findStat', $('#find_su_stat').val(), 30);
...
function _save_client(){
	...
	if (__replace(f.first_iljung_date.value, '-', '') < __replace(f.gaeYakFm.value, '-', '')){
		alert(__getDate(f.gaeYakFm.value)+'이전에 등록된 일정이 있어 기관계약일자 수정이 불가능합니다. 확인하여 주십시오.');
		f.gaeYakFm.focus();
		return;
	}
	...
	if (f.mode.value == 1){
		...
		if (!_save_client_check_sub(f)) return;
		_save_client_sub(f);
	}else{
		if (!_client_check_sugup_data(f)){
			if (!_save_client_check_sub(f)) return;
			_save_client_sub(f);
			return;
		}
...
function _save_client_sub(f){
	if (f.startDate.value.split('.').join('') == f.startDate.tag){
		...
		if (f.mem_change_dt.value == '1'){
			f.mem_change_dt.value = f.startDate.value;
			f.history_yn.value = 'N';
		}
	}
...
// 수급내용 변경 확인
function _client_check_sugup_data(f){
	if (__get_value(f.yLvl)			== __get_tag(f.yLvl)		&&
		...
		f.kupyeo1.value				== f.kupyeo1.tag){

		if (f.yoyangsa1.value == f.yoyangsa1.tag){
			f.mem_change_dt.value = '';
			return false;
		}else{
			f.mem_change_dt.value = '1';
			return true;
		}
	}else{
		f.mem_change_dt.value = '';
...
// 수급내용 되돌리기
function _client_sugup_data_resotre(f){
	for(var i=0; i<f.yLvl.length; i++){
		if (f.yLvl[i].value == f.yLvl[i].tag){
			f.yLvl[i].checked = true;
			break;
		}
	}

	for(var i=0; i<f.sKind.length; i++){
		if (f.sKind[i].value == f.sKind[i].tag){
			f.sKind[i].checked = true;
			break;
		}
	}

	for(var i=0; i<f.sugupStatus.length; i++){
		if (f.sugupStatus[i].value == f.sugupStatus[i].tag){
			f.sugupStatus[i].checked = true;
			break;
		}
	}
...
// 변경사항 확인
function _save_client_check_sub(f){
	if (f.mode.value == 1){
		...
		if (!_check_ssn('su', f.jumin1, f.jumin2, f.code)) return false;
	}

	if (!__alert(f.name)) return false;
	...
	*/
	...
	var stat = __get_value(f.sugupStatus);
...
function _show_save_client(){
	if (document.sugupCheck.startDate.value.split('.').join('') < document.sugupCheck.startDate.tag){
		alert('적용시작일은 현재의 적용시작일보다 커야 합니다. 확인하여 주십시오.');
		document.sugupCheck.startDate.focus();
		return;
	}
...
function _set_kind(kind){
	...
	if (voucher_row != null){
		var row = document.getElementsByName('kind_list[]');
		var row_cnt = 0;

		for(var i=0; i<row.length; i++){
			if (parseInt(row[i].value, 10) >= 1 && parseInt(row[i].value, 10) <= 4){
				if (row[i].checked) row_cnt ++;
			}
		}

		if (row_cnt == 0) row_cnt = 1;

		voucher_row.setAttribute('rowSpan', row_cnt);
	}
	*/
}
...
function _show_pay_layer(gubun){
	...
	if (gubun == '3'){
		document.getElementById('ybnpay').disabled = false;
	}else{
		document.getElementById('ybnpay').disabled = true;
	}
...
//동거가족케어 활성화 레이어 여부
function _famcare_pay_layer(gubun){
	var value = new Array();
	...
	_show_tbody_layer('tbody_5_1', 'layer_5_1', value[1][1]);
	_show_tbody_layer('tbody_5_2', 'layer_5_2', value[1][2]);

	_show_tbody_layer('tbody_6_1', 'layer_6_1', value[2][1]);
	_show_tbody_layer('tbody_6_2', 'layer_6_2', value[2][2]);

	_show_tbody_layer('tbody_7_1', 'layer_7_1', value[3][1]);
	_show_tbody_layer('tbody_7_2', 'layer_7_2', value[3][2]);
	
}

// 수급자 기관계약일자 확인
function _client_cont_date(stat, p_date_from, p_date_to){
...
function _set_max_pay(object, code){
	var kind  = document.getElementsByName('sKind');
	var today = new Date();
...
function _set_my_yul(code, tag){
	var mode	 = document.getElementById('mode');
	var level    = __get_value(document.getElementsByName('yLvl'));
	var boninYul = document.getElementById('boninYul');
	...
	if (level == '9' || code == '3'){ //기초 //if (code == '4'){
		...
		if (code == '3'){
			...
			boninYul.onfocus = function(){
				document.getElementById('kupyeo1').focus();
				this.style.borderColor='';
			}
		}
	}else{ //의료, 경감, 일반
	...
	if (level == '9'){ //등급외
		...
		if (code == '3'){ //기초
			...
			if (mode.value != '1' && code == tag && boninYul.tag != '100.0'){ //기존의 값으로 돌림
...
function _set_my_pay(){
	var mode	  = document.getElementById('mode');
	var kind      = __get_value(document.getElementsByName('sKind'));
	var kind_tag  = __get_tag(document.getElementsByName('sKind'));
	var level     = __get_value(document.getElementsByName('yLvl'));
	var level_tag = __get_tag(document.getElementsByName('yLvl'));
	var kypyeoMax = __commaUnset(document.getElementById('kupyeoMax').value);
	var boninYul  = document.getElementById('boninYul');
	var obj_pay1  = document.getElementById('kupyeo1');
	var obj_pay2  = document.getElementById('kupyeo2');

	if (mode.value != '1' && kind == kind_tag && level == level_tag){
		var kypyeo1 = __commaUnset(obj_pay1.value); //cutOff(parseInt(kypyeoMax) - kypyeo2);
	}else{
		var kypyeo1 = __commaUnset(kypyeoMax); //cutOff(parseInt(kypyeoMax) - kypyeo2);
	}
...
// 정부지원금 최소, 최대금액확인
function _check_min_max_pay(){
	var pay_max = parseFloat(__commaUnset(document.getElementById('kupyeoMax').value), 10);
	var pay_min = pay_max - parseFloat(pay_max * 0.15, 10);
	var pay_now = document.getElementById('kupyeo1');

	var result = true;
	
	/*
	if (parseFloat(pay_now.value, 10) < pay_min){
		alert('최소 정부지원금은 '+__commaSet(pay_min)+'입니다. 확인 후 다시 입력하여 주십시오.');
		result = false;
	}

	if (parseFloat(pay_now.value, 10) > pay_max){
		alert('최대 정부지원금은 '+__commaSet(pay_max)+'입니다. 확인 후 다시 입력하여 주십시오.');
		result = false;
	}
	*/
	...
	if (result){
		...
		if (kind == kind_tag && level == level_tag){
			pay_now.value = __commaUnset(pay_now.tag);
		}else{
			pay_now.value = document.getElementById('kupyeoMax').value;
		}
 ...
 * 기관약정휴일
 */
function _show_center_holiday(){
	var modal = showModalDialog('holiday.php', window, 'dialogWidth:500px; dialogHeight:300px; dialogHide:yes; scroll:no; status:no');
	//window.open('holiday.php');
}

/*
 * 도움말 출력
 */
function _show_help(object, message){
	var body   = __getObject(object);
	var target = __getObject('info_layer_body');
	var draw   = __getObject('info_draw_body');
	
	draw.innerHTML = '<table style="width:200px; background-color:#fff; border:1px solid #ccc;">'
				   + '<tr>'
				   + '<td><p style="text-align:justify; padding:5px; line-height:1.3em;">'+message+'</p></td>'
				   + '</tr>'
				   + '</table>';

	target.style.left = __getObjectLeft(body) + body.offsetWidth + 6;
	target.style.top  = __getObjectTop(body)  + 4;
	target.style.display = '';
}

function _hidden_help(){
	var target = __getObject('info_layer_body');
...
*********************************************************/
function _iesChanged(kind, seq){
	var objModal = new Object();
	var url      = './center_ies_reg.php';
	var style    = 'dialogWidth:500px; dialogHeight:500px; dialogHide:yes; scroll:no; status:no';

	objModal.win  = window;
	objModal.code = $('#mCode').attr('value');
	objModal.kind = kind;
	objModal.seq  = seq;
...
// 직원리스트 엑셀보여주기 체크 팝업
function _yoy_chk_popup(p_cname, p_name, p_phone, p_stat, p_dept, p_ssn){
	var width  = 500;
	var height = 665;
	var left = (window.screen.width  - width)  / 2;
	var top  = (window.screen.height - height) / 2;
	
	popupWork = window.open('yoy_chk_popup.php?fCname='+p_cname+'&fName='+p_name+'&fPhone='+p_phone+'&fStat='+p_stat+'&fDept='+p_dept+'&fSsn='+p_ssn, 'POPUP_WORK_DETAIL', 'left='+left+', top='+top+', width='+width+', height='+height+', toolbar=no, location=no, status=no, menubar=no, scrollbars=no, resizable=no');
}

// 수급자리스트 엑셀보여주기 체크 팝업
function _sugup_chk_popup(p_code, p_cname, p_kind, p_name, p_phone, p_stat, p_ssn, sst, sod, sfl, team){
	var width  = 440;
	var height = 665;
	...
	popupSugup = window.open('sugup_chk_popup.php?fCode='+p_code+'&fCname='+p_cname+'&fKind='+p_kind+'&fName='+p_name+'&fPhone='+p_phone+'&fStat='+p_stat+'&fSsn='+p_ssn+'&sst='+sst+'&sod='+sod+'&sfl='+sfl+'&team='+team, 'POPUP_WORK_DETAIL', 'left='+left+', top='+top+', width='+width+', height='+height+', toolbar=no, location=no, status=no, menubar=no, scrollbars=no, resizable=no');
}
...
document.write('<div id="layer_5_1" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="layer_5_2" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="layer_6_1" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="layer_6_2" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="layer_7_1" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="layer_7_2" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
...
document.write('<div id="tbody_row_layer_1_3" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="tbody_row_layer_1_4" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
...
document.write('<div id="tbody_row_layer_2_4" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="tbody_row_layer_2_5" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="tbody_row_layer_2_6" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
...
document.write('<div id="tbody_row_layer_3_4" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="tbody_row_layer_3_5" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="tbody_row_layer_3_6" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
...
document.write('<div id="tbody_row_layer_4_4" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="tbody_row_layer_4_5" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="tbody_row_layer_4_6" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
...
document.write('<div id="tbody_row_layer_5_4" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="tbody_row_layer_5_5" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');
document.write('<div id="tbody_row_layer_5_6" style="z-index:1010; position:absolute; filter:progid:DXImageTransform.Microsoft.Alpha( Opacity=50, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100); background-color:black; display:none;"></div>');


/* 재무회계서비스 레이어팝업 */
document.write("<div id='divpop1' class='layer_popup' style='position:absolute; width:205px; height:135px; left:450px; top:180; z-index:200; display:none;'>");
document.write("	<table style='width:100%; height:100%;' cellpadding='2' cellspacing='0'>");
document.write("		<tr>");
document.write("			<td align='center' bgcolor='white'><img src='../popup/img/labor_pop.jpg' alt='재무회계서비스' /></td>");
document.write("		</tr>");
document.write("		<tr>");
document.write("			<td align='right' bgcolor='white' style='border:0;' >");
document.write("				<input type='checkbox' name='chkbox' value='divpop1' onclick='closeLayer(true,\"divpop1\");' style='width:21px; border:0;'>오늘 하루 창을 열지 않음");
document.write("				<a href='#' value='divpop1' onclick='closeLayer(false,\"divpop1\"); return false;'><B>[닫기]</B></a>");
document.write("			</td>");
document.write("		</tr>");

...
// 요양사리스트
function getYoyangsaList(p_body, p_code, p_kind, p_year, p_month, p_gubun, p_index, p_center){
	...
	var xmlhttp = new Ajax.Request(
		URL, {
			...
			onSuccess:function (responseHttpObj) {
				...
				if (p_gubun == 'PP'){
					allPayrollConf(p_index, p_center);
				}
			}
		}
	);
...
function payrollSearchTest(p_code, p_kind, p_year, p_month, p_yoyangsa, p_index, p_center){
	getYoyangsaList(myYoyangsa, p_code, p_kind, p_year, p_month, 'PP', p_index, p_center);
...
// 급여 일괄처리
function allPayrollConf(p_index, p_center, p_type){
	var body		= '';
	var count		= document.getElementById('yoyCount').value;
	...
	// 전체 요양보호사 리스트...
	for(var i=0; i<count; i++){
		yoyCode += objectCode[i].value+';;';
		yoyType += objectType[i].value+';;';
	}
	*/
	...
	var xmlhttp = new Ajax.Request(
		URL, {
			...
			onSuccess:function (responseHttpObj) {
				...
				//document.getElementById('yoy_0').style.textDecoration = 'underline';
				...
				/*
				if (p_index != 'all'){
					if (p_index == '' || p_index == undefined) p_index = '0';
					_movePayroll(p_index);
				}
				*/
			}
		}
...
Payroll.prototype.BasePay = function(){
	if (this.type != '3'){
		this.basePay = Math.floor(parseFloat(this.minHourly) * parseFloat(this.workHour));
	}
	return this.basePay;
}
...
Payroll.prototype.ProlongSudang = function(){
	if (this.payType == '1' && this.familyCare == '0'){
		...
		this.prolongSudang = Math.floor(parseFloat(this.hourly) * parseFloat(this.prolongHour));
	}else{
		...
		this.prolongSudang = Math.floor(this.prolongSudang);
	}
...
Payroll.prototype.NightSudang = function(){
	if (this.payType == '1' && this.familyCare == '0'){
		...
		this.nightSudang = Math.floor(parseFloat(this.hourly) * parseFloat(this.nightHour));
	}else{
		this.nightSudang = 0;
		...
		this.nightSudang = Math.floor(this.nightSudang);
	}
...
Payroll.prototype.HolidaySudang = function(){
	if (this.payType == '1' && this.familyCare == '0'){
		...
		this.holidaySudang = Math.floor(parseFloat(this.hourly) * parseFloat(this.holidayHour));
	}else{
		...
		this.holidaySudang = Math.floor(this.holidaySudang);
	}

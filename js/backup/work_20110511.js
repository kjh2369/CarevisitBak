...
function _set_conf_proc_time(index){
	...
	*/
	...
	if (svcCode == '200'){
		...
		var sugaPrice   = parseInt(getHttpRequest('../inc/_check.php?gubun=getSugaPrice&mCode='+document.f.code.value+'&mSuga='+suga_code)); //단가
		...
		if (timeP == 9){
			...
			while(1){
				...
				tempValue[tempIndex] = parseInt(getHttpRequest('../inc/_check.php?gubun=getSugaPrice&mCode='+document.f.code.value+'&mSuga='+sugaCode+tempK)); 
...
function _set_plan_to_conf(index){
	if (index == 'all'){
		if (!confirm('전체볼사를 실해하시면 실적데이타가 계획데이타로 입력됩니다.\n전체복사를 실행하시겠습니까?')) return;
...
function _set_conf_to_cancel(index){
	...
	//if (conf_from[index].value == '' || conf_to[index].value == '') return;

	if (cancel_flag[index].value == 'N'){
		if (status_gbn[index].value != '1'){
			conf_from[index].value      = '';
			conf_to[index].value        = '';
			conf_time[index].value      = '0';
			change_flag[index].value    = 'N';
			document.getElementsByName('suga_name[]')[index].innerHTML  = '0';
			document.getElementsByName('suga_value[]')[index].innerHTML = '0';
		}else{
...
function _work_confirm(code, year, month, mode){
	...
	if (mode == '1'){
		var conf_dt = document.getElementById('conf_dt').value;
		
		if (conf_dt > getToday()){
			alert('실적확정실행일자는 실적마감일자보다 커야합니다. 확인하여 주십시오.');
			return;
		}
		if (!confirm(year+'년 '+month+'월의 실적확정을 수동실행하시겠습니까?')) return;

		var win = window.open('../work/result_confirm.php?pos=2&code='+code+'&year='+year+'&month='+month+'&gubun='+mode, 'WORK_CONFIRM', 'left='+l+', top='+t+', width='+w+', height='+h+', toolbar=no, location=no, status=yes, menubar=no, scrollbars=yes, resizable=no');

...
function _setNeedTime(){
	...
	if (document.f.togeUmu.checked){
		if (document.f.procTime.options[document.f.procTime.selectedIndex].index > 2){
			msg_type = 2;
		}else{
			msg_type = 0;
		}
...
//	}
	...
	if (TN == 9){
		...
		*/
		...
		while(1){
			...
			tempValue[tempIndex] = parseInt(getHttpRequest('../inc/_check.php?gubun=getSugaPrice&mCode='+document.f.mCode.value+'&mSuga='+sugaGubun+tempK)); 
	...
	if (Hgubun != 'Y'){ //휴일이 아닌경우만 할증을 실행한다.
		if (NTNtime > 0){
			/*
...
function _clearDiary(pDay, pIndex){
	...
	*/
	...
	if (temp_add_suga){
		...
		_set_client_month_amount('delete', temp_old_svc, temp_new_svc, temp_old_suga, temp_new_suga, temp_old_bipay, temp_new_bipay);
	}
}
	...
	var xmlhttp = new Ajax.Request(
		URL, {
			...
			onSuccess:function (responseHttpObj) {
				...
				_init_client_month_amount();

				var now_date = calYear+'-'+(calMonth<10?'0':'')+calMonth+'-01';
				var end_date = addDate('d', -1, addDate('m', 1, now_date));
				var last_day = getDay(end_date);

				for(var i=1; i<=last_day; i++){
					var weekday = getWeekDay(calYear+'-'+(calMonth<10?'0':'')+calMonth+'-'+(i<10?'0':'')+i) - 1;
					var object  = document.getElementById('weekDay'+weekday);
					...
					if (object.checked){
						...
						_set_client_month_amount('insert', temp_old_svc, temp_new_svc, temp_old_suga, temp_new_suga, temp_old_bipay, temp_new_bipay);
					}
				}
			}
		}
...
function _addCalendar(){
	...
	if (temp_add_suga){
		_set_client_month_amount('add', temp_old_svc, temp_new_svc, temp_old_suga, temp_new_suga, temp_old_bipay, temp_new_bipay);
	}
}
...
function _modifyCalendar(){
	...
	if (temp_add_suga){
		_set_client_month_amount('modify', temp_old_svc, temp_new_svc, temp_old_suga, temp_new_suga, temp_old_bipay, temp_new_bipay);
	}
}
...
// 수급자 월수급 현황 수정
function _set_client_month_amount(type, old_svc, new_svc, old_suga, new_suga, old_bipay, new_bipay){
...
// 수급자 월수급 초기화
function _init_client_month_amount(){
	var obj_opener = document;

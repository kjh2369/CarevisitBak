	...
	*/
	...
	$sql = 'SELECT	*
			FROM	(
					...
					,		b.nm3 AS svc_nm
					,		a.suga_nm AS sub_nm
					,		COUNT(DISTINCT c.jumin) + SUM(IFNULL(d.cnt_per, 0)) AS cnt_per
					,		COUNT(c.jumin) + SUM(IFNULL(d.cnt_svc, 0)) AS cnt_svc
					FROM	(
					...
					LEFT JOIN (
							SELECT	suga_cd AS cd, DATE_FORMAT(reg_dt, \'%Y%m%d\') AS date, SUM(att_cnt) AS cnt_per, COUNT(suga_cd) AS cnt_svc
							FROM	care_rpt
							WHERE	org_no	 = \''.$orgNo.'\'
							AND		org_sr	 = \''.$SR.'\'
							AND		reg_dt	>= \''.$fromDt.'\'
							...
							ON d.cd		= CONCAT(a.suga_cd, a.suga_sub)
							AND d.date	BETWEEN a.from_dt AND a.to_dt

<?
	...
	foreach($data as $key => $R){
		...
		$sheet->getColumnDimension('D')->setWidth(11.4+$signFirstW);
		$sheet->getColumnDimension('E')->setWidth(4);

		for($i=0; $i<$signCnt; $i++){
			$sheet->getColumnDimension($col[$i][0])->setWidth(3.4);
			$sheet->getColumnDimension($col[$i][1])->setWidth(3.4);
			$sheet->getColumnDimension($col[$i][2])->setWidth(3.4);
		}
		...
		for($i=0; $i<$signCnt; $i++){
			$cellF = $col[$i][0];
			$cellT = $col[$i][2];
			$sheet->getCell($cellF.$rowNo)->setValue($signTxt[$i]);
			$sheet->getStyle($cellF.$rowNo)->getAlignment()->setWrapText(true);
	...
	function SetImage($sheet, $fileImg, $coord){
		...
		//if ($objDrawing->getWidth() > 130){
			$objDrawing->setWidth(130);
		//}

		if ($objDrawing->getHeight() > 103){
			$objDrawing->setHeight(103);
		}

<?
	...
	if($month){
		...
		header("Cache-Control: must-revalidate, post-check=0,pre-check=0" );
		header("Pragma: public" );
		header("Content-Disposition: attachment; filename=".$myF->euckr($name.'대상자 '.$year.'년 서비스내역').".xls" );
	}
				...
				for($m=$fromM; $m<=$toM; $m++){
					...
					/** 타이틀 *******************************************************/
						...
						if($month){
							...
							$sheet->getCell($cellF.$rowNo)->setValue($name.'대상자 '.$year.'년 서비스 내역');
						}
					...
					/** 데이타 *******************************************************/
						if($month){
							...
							$data['TITLE'][] = DefaultData(Array('from'=>'A','to'=>'C','val'=>'월','bgcolor'=>'EAEAEA'));
							$data['TITLE'][] = DefaultData(Array('from'=>'D','to'=>'F','val'=>'일자','bgcolor'=>'EAEAEA'));
							$data['TITLE'][] = DefaultData(Array('from'=>'G','to'=>'Q','val'=>'서비스','bgcolor'=>'EAEAEA'));
							$data['TITLE'][] = DefaultData(Array('from'=>'R','to'=>'AA','val'=>'자원','bgcolor'=>'EAEAEA'));
							$data['TITLE'][] = DefaultData(Array('from'=>'AB','to'=>'AG','val'=>'담당자','bgcolor'=>'EAEAEA'));
							$data['TITLE'][] = DefaultData(Array('from'=>'AH','to'=>'AY','val'=>'비고','bgcolor'=>'EAEAEA'));
						}
						...
						*/
						...
						$sql = 'SELECT	t01_sugup_date AS date
								...
								AND		t01_status_gbn	= \'1\'
								AND		t01_del_yn		= \'N\'
								AND		LEFT(t01_sugup_date,6) = \''.$year.($m < 10 ? '0' : '').$m.'\'
								ORDER	BY date DESC, suga_nm, res_nm, mem_nm';
						*/
						...
						for($i=0; $i<$rowCnt; $i++){
							...
							if($month){
								...
								$data[$i][] = DefaultData(Array('from'=>'AG','to'=>'AY','align'=>'L','border'=>$border,'val'=>StripSlashes($content)));
							}else {
								$data[$i][] = DefaultData(Array('from'=>'A','to'=>'C','align'=>'R','border'=>$border,'val'=>IntVal($row['month']).'월'));
								$data[$i][] = DefaultData(Array('from'=>'D','to'=>'F','align'=>'R','border'=>$border,'val'=>IntVal($row['day']).'('.$weekly[$row['week']].')'));
								$data[$i][] = DefaultData(Array('from'=>'G','to'=>'Q','align'=>'L','border'=>$border,'val'=>$row['suga_nm']));
								$data[$i][] = DefaultData(Array('from'=>'R','to'=>'AA','align'=>'L','border'=>$border,'val'=>$row['res_nm']));
								$data[$i][] = DefaultData(Array('from'=>'AB','to'=>'AG','align'=>'L','border'=>$border,'val'=>$row['mem_nm']));
								$data[$i][] = DefaultData(Array('from'=>'AH','to'=>'AY','align'=>'L','border'=>$border,'val'=>StripSlashes($log[$row['jumin']][$row['mkind']][$row['date']][$row['suga_cd']][$row['mem_cd1']][$row['mem_cd2']]['contents'])));
							}
							/*
								$data[$i][] = DefaultData(Array('from'=>'A','to'=>'D','align'=>'R','border'=>$border,'val'=>IntVal($row['day']).'('.$weekly[$row['week']].')'));

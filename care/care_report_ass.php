				...
				for(var i=0; i<row.length; i++){
					if (row[i]){
						...
						if (col['proCnt'] > 0){
							...
							html += '<td class="center sum bold" style="background-color:#FFFF00;"><div id="lblTotTarget_'+col['suga'].substring(0,3)+'" class="right">0</div></td>';
							html += '<td class="center sum bold" style="background-color:#FFFF00;"><div id="lblTot1Quarter_'+col['suga'].substring(0,3)+'" class="right">0</div></td>';
							html += '<td class="center sum bold" style="background-color:#FFFF00;"><div id="lblTot2Quarter_'+col['suga'].substring(0,3)+'" class="right">0</div></td>';
							html += '<td class="center sum bold" style="background-color:#FFFF00;"><div id="lblTot3Quarter_'+col['suga'].substring(0,3)+'" class="right">0</div></td>';
							html += '<td class="center sum bold" style="background-color:#FFFF00;"><div id="lblTot4Quarter_'+col['suga'].substring(0,3)+'" class="right">0</div></td>';
							html += '<td class="center sum bold" style="background-color:#FFFF00;"><div id="lblTotAll_'+col['suga'].substring(0,3)+'" class="right">0</div></td>';
							html += '<td class="center sum bold" style="background-color:#FFFF00;"><div id="lblTotRate_'+col['suga'].substring(0,3)+'" class="right">0</div></td>';
						...
						rate = __round(tot / (col['target'] > 0 ? __str2num(col['target']) : 1) * 100,1,false);
						rate = (rate > 0 ? rate : '');

						sum['Target_'+col['suga'].substring(0,3)] = __str2num(sum['Target_'+col['suga'].substring(0,3)]) + __str2num(col['target']);
						sum['1Quarter_'+col['suga'].substring(0,3)] = __str2num(sum['1Quarter_'+col['suga'].substring(0,3)]) + __str2num(cnt[1]);
						sum['2Quarter_'+col['suga'].substring(0,3)] = __str2num(sum['2Quarter_'+col['suga'].substring(0,3)]) + __str2num(cnt[2]);
						sum['3Quarter_'+col['suga'].substring(0,3)] = __str2num(sum['3Quarter_'+col['suga'].substring(0,3)]) + __str2num(cnt[3]);
						sum['4Quarter_'+col['suga'].substring(0,3)] = __str2num(sum['4Quarter_'+col['suga'].substring(0,3)]) + __str2num(cnt[4]);
						sum['All_'+col['suga'].substring(0,3)] = __str2num(sum['All_'+col['suga'].substring(0,3)]) + __str2num(tot);
						sum['Rate_'+col['suga'].substring(0,3)] = __round(__str2num(sum['All_'+col['suga'].substring(0,3)]) / (sum['Target_'+col['suga'].substring(0,3)] ? __str2num(sum['Target_'+col['suga'].substring(0,3)]) : 1) * 100,1,false);
						...
						html += '<td class="center" style="background-color:#FFFF00;"><div class="right bold">'+tot+'</div></td>';
...
<div class="title title_border">세부사업보고서(<?=($sr == 'S' ? '재가지원' : '자원연계');?>)</div>
	...
	<tbody>
		...
		if (!Empty($area)){?>
			<tr>
				...
				<td class="left last" colspan="2"><?=$areaNm;?></td>
			</tr><?
		}else{?>
			<th class="center">지역</th>

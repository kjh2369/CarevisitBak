<?
	...
	$sql = 'SELECT	b.cd1 AS mst_cd
			...
			GROUP	BY b.cd1, b.cd2, b.cd3, a.suga_sub, LEFT(t01_sugup_date,6)
			ORDER	BY mst_cd, pro_cd, svc_cd, sub_nm, yymm';
	...
	for($i=0; $i<$rowCnt; $i++){
		...
		$data[$row['mst_cd']]['DATA'][$row['pro_cd']]['DATA'][$row['svc_cd']]['DATA'][$row['sub_cd']]['DATA']['SUM'] += $row['cnt'];
		$data[$row['mst_cd']]['DATA'][$row['pro_cd']]['DATA'][$row['svc_cd']]['DATA'][$row['sub_cd']]['DATA'][$month] = $row['cnt'];

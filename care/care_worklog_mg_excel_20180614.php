		...
		*/
		...
		/*
			$sql = 'SELECT	quart
					...
					,		dtl_cd
					,		dtl_nm
					,		SUM(svc_cnt) AS svc_cnt
					,		SUM(today_cnt) AS today_cnt
					FROM	(
							SELECT	CASE WHEN a.month >= 1 AND a.month <= 3 THEN 1
										 WHEN a.month >= 4 AND a.month <= 6 THEN 2
										 WHEN a.month >= 7 AND a.month <= 9 THEN 3 ELSE 4 END AS quart
							,		a.pro_cd
							,		a.dtl_cd
							,		b.suga_nm AS dtl_nm
							,		a.svc_cnt
							...
							FROM	(
									...
									,		t01_mkind AS org_type
									,		t01_sugup_date AS date
									,		CAST(SUBSTR(t01_sugup_date,5,2) AS unsigned) AS month
									,		SUBSTR(t01_suga_code1,1,3) AS pro_cd
									,		t01_suga_code1 AS dtl_cd
							...
							INNER	JOIN	care_suga AS b
									...
									AND		CONCAT(b.suga_cd, b.suga_sub)	 = a.dtl_cd
									AND		REPLACE(b.from_dt,\'-\',\'\')	<= a.date
									AND		REPLACE(b.to_dt,\'-\',\'\')		>= a.date
							) AS a
					GROUP	BY quart, pro_cd, dtl_cd
					ORDER	BY dtl_cd, quart';
		 */
		//데이타
		$sql = 'SELECT	quart
				,		pro_cd
				...
				FROM	(
						SELECT	CASE WHEN a.month >= 1 AND a.month <= 3 THEN 1
									 WHEN a.month >= 4 AND a.month <= 6 THEN 2
									 WHEN a.month >= 7 AND a.month <= 9 THEN 3
									 WHEN a.month >= 10 AND a.month <= 12 THEN 4 ELSE 0 END AS quart
						,		a.pro_cd
						,		a.dtl_cd
						,		a.suga_nm AS dtl_nm
						,		a.svc_cnt
						...
						FROM	(
								SELECT	a.org_no
								,		a.suga_sr AS org_type
								,		a.suga_nm
								,		t01_sugup_date AS date
								,		CAST(SUBSTR(t01_sugup_date,5,2) AS unsigned) AS month
								,		SUBSTR(a.suga_cd,1,3) AS pro_cd
								,		CONCAT(a.suga_cd, a.suga_sub) AS dtl_cd
								,		COUNT(t01_suga_code1) AS svc_cnt
								,		SUM(CASE WHEN t01_sugup_date = \''.str_replace('-','',$date).'\' THEN 1 ELSE 0 END) AS today_cnt
								FROM	care_suga AS a
								LEFT	JOIN	t01iljung
										ON		t01_ccode		= a.org_no
										AND		t01_mkind		= a.suga_sr
										AND		t01_status_gbn	= \'1\'
										AND		t01_del_yn		= \'N\'
										AND		t01_suga_code1	= CONCAT(a.suga_cd, a.suga_sub)
										AND		t01_sugup_date <= \''.str_replace('-','',$date).'\'
										AND		LEFT(t01_sugup_date,4) = \''.SubStr($date,0,4).'\'
								WHERE	a.org_no	 = \''.$orgNo.'\'
								AND		a.suga_sr	 = \''.$SR.'\'
								AND		a.from_dt	<= \''.str_replace('-','',$date).'\'
								AND		a.to_dt		>= \''.str_replace('-','',$date).'\'
								GROUP	BY a.suga_cd, a.suga_sub, t01_sugup_date
								) AS a
						) AS a
				GROUP	BY quart, pro_cd, dtl_cd
				ORDER	BY dtl_cd, quart';
		...
		for($i=0; $i<$rowCnt; $i++){
			...
			$tmpR[$row['dtl_cd']]['q'.$row['quart']] = $row['svc_cnt'];
			$tmpR[$row['dtl_cd']]['today'] += $row['today_cnt'];
			$tmpSum[$row['pro_cd']]['val'] += $row['svc_cnt'];

			$data['SUM'][$row['quart']]['val'] += $row['svc_cnt'];
			$data['SUM'][5]['val'] += $row['today_cnt'];
			$data['SUM'][6]['val'] += $row['svc_cnt'];
		}
